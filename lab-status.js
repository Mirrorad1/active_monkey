...
window.AM_LAB_STATUS = {
  latest_exp: { n: 187, title: "identity-n4-crack rung 5, the licensed controller re-test (pre-registered e1bda4e): MIXED — THE CRACK IS CLOSED CONSTRUCTIVELY at normal tolerance by INT-C2900 (9/9 cells, defense 7/8 each, revision in-tolerance: the CONTINUOUS-PRESSURE-STRETCH discriminator works) while BOTH mechanism predictions for the evidence form fail honestly (F1+F3: INT-E600 0/9 — with a named instrument-fidelity gap: the accumulator counted QUIET blocked evidence, so the pre-registered pressure-gated E-form went untested); the universal-constant law is REFINED, not broken: constants suffice ON THE RIGHT SURFACE, and the surface is the maximum continuous-pressure stretch", tag: "MIXED" },
  tally: { total: 187, positive: 108, negative: 48, mixed: 27, breakthroughs: 5 },
  directions: [
    { name: "identity-n4-crack", state: "active (Exp 188 pre-registered — the human's single-regulated-controller retest; chapter synthesis follows it)", latest: "Exp 184 (exploratory squeeze map) + Exp 183 addendum (seed-229 autopsy)", next: "ON RESUMPTION — Part-3 classification of the candidate sets (is any candidate a REAL crack after the class-D dissolution check?), then rung-4 fresh-seed confirmation" },
    ...
    { name: "relational-thoughts", state: "exploratory", latest: "TBD-human", next: "Rung 1 two-hop spatial: creature learns \"object at place P\" and \"P is in room R\"; query \"which room is the object in?\" without co-observing object+room; FAIL = answer at chance" },
    { name: "sequence-substrate", state: "exploratory", latest: "TBD-human", next: "Rung 1 AIF pair-state (trigram) model: reproduce taught Q→A inside active inference proper; FAIL = pair-state model can't reproduce what template already does" },
