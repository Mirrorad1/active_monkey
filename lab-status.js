...
window.AM_LAB_STATUS = {
  latest_exp: { n: 186, title: "identity-n4-crack rung 4, the fresh-seed core confirmation (pre-registered e3e282e): 6/6 CORE CELLS CONFIRM — THE CRACK IS REAL AT THIS BODY (BREAKTHROUGH; blind-verified CONFIRMED, every conjunct recomputed from rows): no arm in the extended config family passes both bars in any core cell while the oracle defends 8/8 and the baseline deficit holds 8/8 everywhere; the V-sample resolved the (1200,1,·) anomaly as noise AND found 3 more cracks; the sharpest single datum: THETA3-H3000 defends 7/8 in (2400,4,600) yet still fails revision — the two-inequality squeeze in one cell", tag: "POSITIVE" },
  tally: { total: 186, positive: 108, negative: 48, mixed: 26, breakthroughs: 5 },
  directions: [
    { name: "identity-n4-crack", state: "active (rung 5 = Exp 187 pre-registered — the intermittency-gated concession vs the confirmed crack cells; kill test runs ON the new surface)", latest: "Exp 184 (exploratory squeeze map) + Exp 183 addendum (seed-229 autopsy)", next: "ON RESUMPTION — Part-3 classification of the candidate sets (is any candidate a REAL crack after the class-D dissolution check?), then rung-4 fresh-seed confirmation" },
