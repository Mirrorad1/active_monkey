...
- **L45 (Exp 271, 2026-06-26).** Before a HEAD-TO-HEAD "kill test" between two mechanisms (does surface A beat surface B where B fails?), PRE-FLIGHT that they can CO-EXIST in one common regime -- a mechanism's "wall"/crack can be SPECIFIC to a regime the competitor cannot even enter, making the contest ILL-POSED (CAN'T-POSE), not winnable. Exp 271: the N4 freeze-vs-revise crack is a HARD-CAPTIVITY phenomenon (freezing only defends when the agent is force-fed with no escape); a movement defense needs a SOFT attack (escape freedom); and the soft attack that creates a real deficit DISSOLVES the freeze surface (best fixed-H defense 0/8) -- so there is no defendable B-peer for A to beat at any A-viable regime. The two surfaces are regime-incompatible; the kill test cannot be staged. RULE: a posability pre-flight (small, 2 cells) with an EQUIVALENCE GATE (the new common-regime runner must reduce to the committed original at the limiting parameter and reproduce its verdict structure -- proving the port faithful, L15) decides feasibility BEFORE the expensive full contest; CAN'T-POSE is a real, reportable boundary finding, not a failure. Kin L28 (pre-flight the premise) + L15 (gates emit evidence). [PROTOCOL step 3; METHODOLOGY; VALIDATION]
- **L46 (Exp 272, 2026-06-27).** POSABLE != LEARNABLE is gated by the EXPLORATION REGIME, not the capability: a defense that an omniscient planner can execute may be unlearnable from experience IF the adversary biases the agent's own exploration away from the knowledge it needs. Exp 272: a movement identity-defense is learnable (model-based: learn the world-map by FREE/peacetime exploration -> plan -> matches optimal, coverage 1.0) but is COVERAGE-CAPPED when the map is learned ONLY under the attack (the pull keeps the agent away from the refuge -> refuge cells visited 0 times -> 8% map error -> capped plan), and a naive model-free Q-learner gets TRAPPED (HOLDs on the attack color, lr=lr0/(1+visits) freezes the high-traffic Q before the escape value propagates). DEBUGGING TECHNIQUE (the isolation that made this clean, kin L13): to separate a learner's PLANNING gap from its KNOWLEDGE-ACQUISITION gap, run the planner on the TRUE model (here MB-on-true-map closure ~1.0) -- if that matches optimal, the planner is correct and the whole gap is acquisition (then check COVERAGE: budget-invariant sub-1.0 coverage = a structural exploration-distribution block, not an under-training/budget issue). RULE: when asking "is X learnable", name the EXPLORATION REGIME the learner is allowed (peacetime/free vs under-attack/biased); a "not learnable" under biased exploration is NOT "fundamentally unlearnable". Scope-flag small substrates (free coverage is trivial on 25 cells). Kin L22/L32 (forced/imposed != evolvable), L44 (myopic planner false negative). [VALIDATION; METHODOLOGY; PROTOCOL step 4.5]
- **L47 (Exp 273, 2026-06-27).** When a learnability/coverage result was shown on a SMALL substrate (L46 scope-flag), TEST SCALING with an ARTIFACT GATE that must FIRE before the result is admissible: predeclare a metric that proves you LEFT the trivial regime (here: at a FIXED budget, free-exploration coverage must DROP below a bar at the largest size), and if it does not fire the verdict is NO-VERDICT-STILL-TRIVIAL, not SCALES -- a CLEAN run can be UNDER-RANGED (Exp 273's first workflow attempt maxed at N=400 where fixed-budget coverage was still 0.955, so its gate could not fire, yet it narrated an unentailed "WALL"; the controller caught it, extended sizes to N=900 where coverage=0.811<0.90, and re-ran). The positive finding: observation-only learnability is BUDGET-PROPORTIONAL -- it survives scaling with a free-exploration budget ~ the random-walk COVER TIME (knee ~16xN_cells), and "the map is the gap" shows as a clean COVERAGE->CLOSURE coupling (closure rises monotonically with coverage; planner-correct at every size via plan-on-true-map ~1.0). Also: a COLOR-BLIND exploration walk's coverage depends ONLY on the move graph, so a far/clustered TARGET cannot block it (claiming so is a category error -- a real exploration block needs move-graph walls/bottlenecks). Kin L46 (exploration-regime), L25 (runtime preflight -- scaling tests must bound compute), L13 (instrument-resolution). [VALIDATION; METHODOLOGY; PROTOCOL step 3/5]
- **L48 (Exp 274, 2026-06-29).** Model-BASED and model-FREE learners can FAIL THE SAME TASK FOR DIFFERENT REASONS, so a learnability verdict must NAME the binding constraint per method -- don't generalize one learner's wall to "the task". L46/L47 established model-based learnability is COVERAGE-gated (map the world by free exploration -> plan exactly -> works/scales). The natural follow-on hypothesis -- "the model-free trap (Exp 272) is also coverage: give it EXPLORING STARTS and it'll learn" -- is REFUTED: exploring starts pin state coverage to 1.000 (covL=covR=1.0) yet the trap color stays unlearned (closure -0.280, 0/8), and 4x budget does NOT lift it (-0.027), while the model-based learner reaches optimal (~1.0) on the identical cells. So tabular-Q learnability here is CONVERGENCE-gated: TD credit-assignment cannot propagate the long-escape value under a discounted (gamma=0.999) pull kernel + an absorbing HOLD-on-attack trap (weak per-action signal under a 50% override, large Q magnitudes). DEBUGGING TECHNIQUE (the isolation that attributes the failure, kin L46): to separate a model-free learner's COVERAGE gap from its CONVERGENCE gap, force coverage to 1.0 with EXPLORING STARTS (random restart) and hold the kernel exact -- any residual gap is convergence, not exploration; cross-check against the model-based learner on the SAME cells (if MB ~1.0, the target is reachable and the gap is method-specific, not a substrate wall). RULE: "X is/ isn't learnable" must say BY WHICH METHOD and name the lever (coverage for model-based; convergence for model-free); proving coverage isn't the lever (budget-invariant covR=1.0) does NOT prove unlearnability -- it relocates the wall to convergence (untested repairs: eligibility traces/TD-lambda, lower gamma, prioritized sweeping, Dyna). A negative scoped to vanilla-Q + exploring-starts + <=4x budget is NOT "model-free impossible". Kin L46 (POSABLE!=LEARNABLE, exploration-regime), L44 (myopic/method-limited false negative), L22/L32 (forced/imposed != evolvable). [VALIDATION; METHODOLOGY; PROTOCOL step 4.5]
- **L49 (Exp 274, 2026-06-29).** NUMBER-KEYED DERIVED ARTIFACTS GO STALE SILENTLY when a research arc is RENUMBERED or DELETED: anything keyed by experiment number (meta/episodes/expNN.json, site entries, caches) keeps the PRIOR occupant's content under the reused slot, and a deleted tail leaves ORPHANS for numbers that no longer have an EXPERIMENTS.md entry. The generators are correct but lazy -- they only refresh a slot when RE-RUN, so the stale JSON/entry rides along into the next commit and misattributes the experiment (here: the deleted sparse-llm arc's exp274 'drift_discrimination' episode collided with the renumbered identity-ecological exp274, and stale 275/276 lingered). RULE: after ANY renumber/delete of EXPERIMENTS.md entries, REGENERATE (never trust) every number-keyed derived artifact for the affected range, and remove orphans. Durable guard (mechanical > prose, loop/META.md): tests/test_meta_monkey.py::test_committed_meta_episodes_are_not_stale_or_orphaned asserts every committed episode (a) maps to an EXPERIMENTS.md entry and (b) has an on-disk script_path -- it fails on both a stale collision and an orphan (negative-tested: planting a bogus exp9999.json reds it). Kin L42 (byte-identical no-op = silently wrong lever), L15/L16 (gates emit evidence / source-patch no-ops), the shared-checkout-concurrency hazard (a live autosync Stop-hook commits the working tree between turns, non-atomically -- it split this very iteration's commit). [METHODOLOGY; VALIDATION; PROTOCOL step 5]
- **L50 (Exp 275, 2026-07-02).** Two instrument traps when pre-flighting a capability on a DEMOGRAPHICALLY
  UNSTABLE substrate (overshoot->crash/extinction). (i) A per-capita OUTCOME metric (intake, carrying-capacity
  N_star) CONFOUNDS the capability with the SURVIVAL trajectory -- an arm can post high TRANSIENT intake then
  go extinct, and you cannot ratio a surviving arm against an extinct one (NaN) -> measure the capability's
  CORE BEHAVIOR directly with a demographic-confound-free metric from a fixed cohort over an early window
  (here IN-BAND OCCUPANCY = fraction of time on the drifting resource band, kin Exp 210 wrong-cell occupancy).
  (ii) A PROXY for a graded capability must be FAITHFUL: `thermal_avoidance_weight` is ARGMAX-SCALE-INVARIANT
  (once >0 the same neighbor wins the argmax, so the sweep is artifactually FLAT and reads a false
  "no range"/CAN'T-POSE), whereas `band_responsiveness` in [0,1] (an EMA tracking rate) genuinely interpolates
  use-quality. Before reading "capability X has no expressible range" off a proxy sweep, verify the proxy
