...
**STATUS.** state: closed-negative (Exp 275–277) · latest: Exp 277 (STALL / NEGATIVE, blind-verified, 2026-07-08) — the Baldwin escape FAILS: within-life theta learning does NOT assimilate innate theta (ON 0.168 <= OFF 0.212; Step-A ~0.7% gap = learning shields the prior; CLAMP≈ON = not substitution) · arc: POSABLE (275) → SOFT rich-regime-only WALL (276) → BALDWIN FAILS (277) · closing answer: the Baldwin mechanism does not escape the local-gradient wall here; the limiter is the SUBSTRATE (benefit saturation + demographic instability) · reusable: heritable+costed band_responsiveness + within-life theta learner (byte-identical OFF); L50/L51 · re-opens only on a NAMED lever (sharper cost, stronger optimizer, or a NON-SATURATING-benefit trait).
