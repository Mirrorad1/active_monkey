...
**STATUS.** state: CLOSED-NEGATIVE (Exp 275–277) · latest: Exp 277 (STALL / NEGATIVE, blind-verified, 2026-07-08) — the Baldwin escape FAILS: within-life theta learning does NOT assimilate innate theta (ON 0.168 <= OFF 0.212; Step-A ~0.7% fitness gap = learning shields the prior; CLAMP≈ON rules out substitution) · arc: POSABLE (275) → SOFT rich-regime-only WALL (276) → BALDWIN FAILS (277) · closing answer: the Baldwin mechanism does not escape the local-gradient wall for a costed sense-USE trait — the limiter is the SUBSTRATE (benefit saturation + demographic instability), not the trait/mechanism · reusable: heritable+costed band_responsiveness + within-life theta learner (both byte-identical OFF); L50/L51 · re-opens only on a NAMED lever (a sharper cost building a real wall+gradient; a stronger optimizer; a NON-SATURATING-benefit trait).
