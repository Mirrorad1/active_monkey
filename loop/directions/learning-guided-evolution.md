...
**STATUS.** state: active (rung 2a done — a SOFT rich-regime-only wall) · latest: Exp 276a (MIXED / NEW INSIGHT, blind-verified, 2026-07-08) — heritable+costed innate theta does NOT robustly invade from rarity (10/24, inv_frac 0.417) so the Baldwin premise stands, BUT posable only in a resource-rich regime (competitive CAN'T-POSE, Exp 242-247) + the theta-info edge is a weak fixation-driven priority tail · reusable: heritable band_responsiveness trait (enable_learnable_use, byte-identical OFF) + L50/L51 · next-falsifiable: Exp 276b — build the within-life theta LEARNER; learn-OFF reproduces the soft wall, learn-ON must climb (PASS/STALL/SHIELDING/SUBSTITUTION).
