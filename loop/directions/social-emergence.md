...
**STATUS.** state: closed-positive (ladder Exp 63–74) · latest: Exp 74 · depends-on: persistent-creature, functional-emergence · reusable: clade infra (vela = first committed peer spine, Exp 63), shared-world substrates, social-transmission metrics · why: one inertia law explained every social effect (value transmission Exp 66 young-receivers-only; dialect convergence Exp 73); NONE undesigned — coordination deflated to stigmergic lock-in (Exp 70–72). CLOSURE: no undesigned social novelty here; descendants: graded-uncertainty (Exp 85–91), self-other-modeling (Exp 228–234) · next-falsifiable: NONE — ladder closed; do NOT re-run Rung 1 (vela exists); descendant `self-other-modeling` carries the open question (model the OTHER's hidden state).
