...
**STATUS.** state: active · R3b DONE (Exp 267, NEGATIVE/NEW INSIGHT): decoupling offense (aggr) from defense (escape) STILL yields no predator/prey roles — converges to a monomorphic GENERALIST APEX (aggr p50 0.97 + escape maxed 2.5-3.9, corr~0, 100% pred/0% prey, 0/10 extinct) at all defense-costs. With R3 (coupled->invulnerable apex): roles don't self-organize from cannibalism+soft costs (optimum is a generalist either way) — specialization needs a HARD trade-off / disruptive selection. ARC: R1 aggression EMERGES (clears wall) · R2 orthogonal to predator · R2-followup refuges decouple · R3 roles need offense/defense decoupling · R3b decoupling insufficient (role wall = no specialization-forcing trade-off). · next: CLOSE the direction, OR R3c hard shared offense/defense budget
