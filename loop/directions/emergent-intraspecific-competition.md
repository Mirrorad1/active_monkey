...
**STATUS.** state: active · R3 DONE (Exp 266, NEGATIVE/NEW INSIGHT): predator/prey ROLES do NOT emerge from single-trait cannibalism — aggr converges to the ceiling (1.000, std 0, 100% predator/0% prey, 0/10 extinct) at all cannibal_cost {0.05,0.10,0.20}; a single predatoriness trait couples OFFENSE + INVULNERABILITY (aggr=1.0 unkillable) -> runaway apex, no stable prey niche. Roles need offense/defense DECOUPLED. ladder: R1 aggression EMERGES (clears wall) · R2 co-emerges but orthogonal · R2-followup refuges decouple it from predation · R3 roles need offense/defense decoupling. substrate: gated enable_contest+enable_cannibalism+aggr+lineage on ecology/patchmosaic.py, byte-identical OFF · next: R3b decouple offense/defense (separate vulnerability trait), OR close the direction
