...
**STATUS.** state: active (NEW, design-stage) · mechanism: permissive costed UNREWARDED `enable_contest` affordance (heritable `aggr`, energy-theft) + lineage tracking on ecology/patchmosaic.py, byte-identical OFF · driver: energy-preservation only; evaluator records/diagnoses, never selects · why: separate 'affordance provided' from 'behavior selected' — does costed intraspecific competition clear the local-gradient wall that closed population-ecology? · depends-on: patchmosaic co-evolution (Exp 257-262), ecology/evolvability/metrics.py, mean-of-opposites-guard · next-falsifiable: R1 — does a heritable prey-vs-prey contest strategy invade from rarity + show a positive LOCAL gradient under food SCARCITY but NOT abundance, lineages observable?
