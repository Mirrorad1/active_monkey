...
**STATUS.** state: active · R2 DONE (Exp 264, MIXED): (A) scarcity basin map clean (P(aggression invades) 9/10 scarce -> 0/10 abundant, transition attack_a~0.04-0.06); (B) intraspecific aggression robustly CO-EMERGES with the co-evolving predator (aggr@end 0.857, all 10 seeds — NOT washed out, refuges supply local crowding) but is ORTHOGONAL — contest ON vs OFF indistinguishable on escape (both bimodal ~1.2-1.5)/attack(~2.0)/coexistence(0/10 extinct); does NOT buffer the Exp-259 collapse. R1 (Exp 263): aggression EMERGES under scarcity, clears the local-gradient wall. substrate: gated enable_contest+aggr+lineage on ecology/patchmosaic.py, byte-identical OFF · next: R3 cannibalism -> emergent ROLES (what is a predator/prey); optional refuge-free R2 follow-up
