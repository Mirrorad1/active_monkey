...
**STATUS.** state: closed-positive · closed: Exp 267 (five-rung arc) · arc: a permissive costed UNREWARDED affordance makes intraspecific competition EMERGE with no hard-coding — R1 prey-vs-prey aggression SELECTED under scarcity, clears the local-gradient wall that closed population-ecology → R2 orthogonal to a co-evolving predator → R2-followup REFUGES decouple it from predation → R3/R3b emergent predator/prey ROLES do NOT self-organize from cannibalism (coupled & decoupled both -> a monomorphic apex): roles need a specialization-forcing HARD trade-off the soft-cost substrate lacks · reusable: patchmosaic.py gated enable_contest+enable_cannibalism+aggr+lineage (byte-identical OFF), mean-of-opposites-guard · next: re-open on a word — R3c hard offense/defense budget
