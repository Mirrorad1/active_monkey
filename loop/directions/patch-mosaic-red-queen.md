...
**STATUS.** state: active · latest: Exp 261 (costed predator attack — MIXED: a clean byte-isolated CAUSAL lever — confirms the Exp-260 asymmetry caused Exp-259 dominance, predator gains an interior ESS once costed, ac=0→pred-dom +0.917, ac=0.40→prey-dom -1.909 — but does NOT symmetrize: the costed-vs-costed Red Queen is BISTABLE (intermediate sd(D) 0.62-0.74, per-seed prey-railed vs prey-collapsed); draft 'symmetrized' RETRACTED by adversarial verification + per-seed re-run) · ladder: 257 CAN_POSE · 258 robust (POSITIVE-PARTIAL) · 259 Red Queen→predator-dominance (POSITIVE-SINGLE) · 260 prey climb weak/ESS-bounded (MIXED) · 261 symmetric costs→bistability not symmetry (MIXED) · next: characterize the bistability (longer horizon/higher ceiling; basin map) OR close the direction
