...
**STATUS.** state: closed-positive · closed: Exp 262 · arc: a discrete patch mosaic POSES global predator-prey coexistence where the homogeneous arena CAN'T (257 CAN_POSE → 258 robust topology×scale) and hosts a discrete Red Queen (259 ENGAGES as predator-dominance → 260 costed-prey/costless-predator asymmetry is the mechanism → 261 that asymmetry is the causal lever but symmetric costs give BISTABILITY not symmetry → 262 bistability REAL + attack_cost tunes the basin probability) · reusable: ecology/patchmosaic.py (gated attack_cost+escape_cost, byte-identical OFF), ecology/evolvability/metrics.py, skill mean-of-opposites-guard · why: homogeneous CAN'T-POSE wall was spatial synchronization, not discreteness · next: re-open only on a word — invasibility/ESS check
