...
**STATUS.** state: active (READY TO CLOSE) · latest: Exp 262 — bistability characterized (POS-SINGLE): the Exp-261 bistability is REAL, robust to trait_max 4→8 + horizon 3000→5000 (intermediate sd(D) 0.75-0.79, opposite-regime per-seed splits, 0 extinctions); attack_cost tunes P(prey-dom) as a sigmoid 0.00→1.00 (50/50 at ac~0.10-0.15) → TWO stable attractors, not symmetric coexistence · ladder: 257 CAN_POSE → 258 robust (POS-PARTIAL) → 259 Red Queen=predator-dom (POS-SINGLE) → 260 prey gradient weak/ESS-bounded (MIXED) → 261 symmetric costs→bistability not symmetry (MIXED) → 262 bistability real + basin-tuned (POS-SINGLE) · next: CLOSE the chapter; optional invasibility/ESS check at the joint resident
