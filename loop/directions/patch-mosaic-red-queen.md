...
**STATUS.** state: active · latest: Exp 257 (patch-mosaic posing preflight — substrate build in progress) · depends-on: Exp 248–256 predator-prey toolkit + ecology/wellmixed.py (branch exp235-terrain-locomotion, pre-main) · reusable: predation engine, logistic prey, Type III, predator interference, predator self-limit, wellmixed within-patch dynamics · why: homogeneous spatial arena closed CAN'T-POSE via substrate synchronization/destabilization; test whether patch asynchrony+refugia+migration rescue global coexistence · next-falsifiable: Exp 257 — does intermediate-migration + asynchronous patches pose bounded global predator-prey coexistence where the homogeneous arena could not?
