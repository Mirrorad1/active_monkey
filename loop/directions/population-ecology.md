...
**STATUS.** state: active · latest: Exp 201 (NEGATIVE / NEW INSIGHT — a WALL, unanimously blind-verified 3/3, branch exp201-increasing-returns → PR — the INCREASING-RETURNS escape TESTED and FAILED: forcing precision to TRACK a moving food band (per-creature intensity-keyed EMA tracker replacing exp200's free read) does NOT cross the valley. Fresh seeds {33-37}: FAST gene-pool newborn intensity 0.128 (4/5 <0.15, 0/5 >0.30), MEDIUM 0.116, SLOW null 0.108, SATURATING free-read 0.083, USELESS 0.053; P1+P2 hold, 0 extinct, internal degeneracy clean. A REAL but SUB-THRESHOLD speed-gated gradient exists (FAST>MEDIUM>SLOW, FAST tracked > SATURATING free-read) — the first non-flat null in the arc — but it never pays: a frozen-precision returns probe shows the benefit of precision is CONCAVE (marginal +0.043→+0.018, convex_above_0.30=False) and dominated by the linear cost, and the moving band LOWERS everyone's intake (FAST intake < SLOW at every p). So 'increasing-returns geometry' did NOT produce increasing RETURNS — NO GOLDILOCKS GRADIENT, a third time. The ceiling is now general across avoidance (199) + foraging (200) + increasing-returns/moving-target (201). Lesson L23 (pilot at the full verdict horizon; a transient peak over-states the equilibrium). Untested: a regime with REAL frequency dependence (precision pays MORE as rivals also sense; interference competition was weak here, strip≈0)) · depends-on: ecology/ substrate (Exp 194–201) · reusable: ecology engine (+ band-staleness per-creature tracker, frozen-precision returns probe) · why: opens the population axis distinct from the locked ladder (human's "N5 population ecology" = ladder-N7 territory, kept parallel) · next-falsifiable: AWAITING HUMAN STEER — the 199+200+201 wall closes "does a costed precision-organ evolve functional" across avoidance, foraging AND increasing-returns geometry. Options: (a) the LAST sensing escape — a regime with REAL frequency dependence / interference competition (precision pays more as rivals also sense; make the band genuinely depleting so being first matters); (b) accept the wall, move to a different N5 axis (niche/species archive = sympatric divergence, or mate choice / assortative reproduction); (c) re-run Exp 198 with the L21 validity fix to clean-grade the attractor POSITIVE; (d) neural Bθ; (e) redirect to the moonshot main line.
