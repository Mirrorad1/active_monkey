...
**Substrate-engineering arc close (Exp 243–246).** The toy continuous substrate is WEAKLY-POSABLE (stable A-regulated equilibrium; real-but-weak ~5–13% density-dependent competition) but DEMOGRAPHIC STABILITY and STRONG RESOURCE COMPETITION are MUTUALLY EXCLUSIVE on it. Unifying trade-off (Exp 246 capstone): strong competition requires a resource-limited population (per-capita intake ~ cost), but resource-limited dynamics OSCILLATE (lagged energy-buffer feedback, Exp 242). Mechanism A damps oscillations by capping N below resource-limitation → stable, but competition stays weak. Five levers all NEGATIVE: founder, A-strength, regen, bump geometry (Exp 243–245), moving patch (Exp 246). The certification harness (ecology/evolvability/stability.py + cert_run.py) caught four instrument artifacts before any escape was published. Only named escape is out of scope: a consumer with NO energy buffer (agent redesign); re-open on a word. Full detail: EXPERIMENTS.md Exp 243–246.

**STATUS.** state: closed-negative · closed: Exp 246 (substrate-engineering arc; five levers exhausted) · reusable: ecology/evolvability/stability.py + cert_run.py (oscillation detector + non-degeneracy audit) · why: STABILITY vs STRONG COMPETITION mutually exclusive on this substrate; discrete local-gradient wall stands · next: re-open only on a word — agent-architecture redesign (no-energy-buffer consumer)
