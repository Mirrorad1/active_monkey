...
**STATUS.** state: active · latest: predeclared (no experiment yet; gridworld locomotion CLOSED Exp 235–237) · depends-on: environmental-complexity (wall extends to discrete locomotion), continuous-creature/nira, the FREQUENCY_DEPENDENT-aware Preflight, the Exp 237 food-sense pattern · reusable: the wall arc instrument set + L39 (expressibility-first) + L40 (anti-gaming re-runs) + L41 (invasion-from-rarity binding) · why: continuous movement physics is the one substrate where a locomotion benefit could be NON-saturating and individually-accruing — the escape condition the discrete grid never met · next-falsifiable: Rung 1 — the gen-0 monomorphic benefit curve of a costed continuous locomotion trait is FLAT/saturating = no escape, the wall is substrate-general.
