...
**STATUS.** state: closed-negative · closed: Exp 243 (confirmed-with-mechanism) — the toy continuous substrate CANNOT host a non-degenerate locomotion-evolvability equilibrium: runaway when intake ignores depletion (Exp 238-241, foundational intake bug), oscillatory when it does not (Exp 242); Exp 243 added crowding mortality (Mechanism A) that damps oscillations but only on a SATURATED field (availability 0.98–0.99 >> 0.85 ceiling), degenerate by the certify_run L40 audit; 0/24 cells non-degenerate across A-strength + regen depletion sweeps; viability↔depletion tension is BINDING for this geometry (viable population is too sparse to deplete a 576-sub-cell field). Discrete local-gradient wall STANDS (Exp 199-237). Re-open condition: GEOMETRY redesign (smaller arena / denser population), NOT a calibration tweak — being pursued as Exp 244.
