...
**STATUS.** state: active · latest: Exp 238 MIXED/artifact-suspect — continuous space escapes discrete saturation (bump Spearman 0.943) but the rise is dominated by distance arithmetic (flat-null + nav-off both Spearman 1.000); genuine navigation bonus real but secondary (+0.07 slope); intermediate PASS over-claim caught/self-corrected · next-falsifiable: Rung 1b — isolate navigation vs distance confound (cost-neutralise coverage so only efficient navigation pays), then Rung 2 invasion-from-rarity; do NOT run Rung 2 on current confounded substrate.
