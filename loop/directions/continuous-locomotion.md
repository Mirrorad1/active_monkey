...
**STATUS.** state: CLOSED · closed: Exp 243–246 (substrate-engineering arc complete) — the toy continuous substrate is WEAKLY-POSABLE (a stable A-regulated equilibrium with real-but-weak ~5-13% density-dependent competition across all levers) but CANNOT be engineered to STRONG competition and therefore CANNOT host a clean invasion-from-rarity ESS test. UNIFYING TRADE-OFF (Exp 246 capstone): DEMOGRAPHIC STABILITY AND STRONG RESOURCE COMPETITION ARE MUTUALLY EXCLUSIVE on this substrate. Strong competition requires a resource-limited population (per-capita intake ~ cost), but a resource-limited population on this substrate OSCILLATES (lagged energy-buffer feedback, Exp 242). Mechanism A damps oscillations by capping N BELOW the resource-limited density — so STABLE <=> A-regulated <=> below resource-limitation <=> weak competition. Five levers tested, all NEGATIVE: founder calibration, A-strength (hmax), regen rate (Exp 243), bump geometry (Exp 245), moving/dynamic patch (Exp 246). The ~+43% speed benefit (Exp 240) is real but unbounded by weak competition — directional/runaway invasion rather than a clean ESS (the Exp-240 commons tragedy now mechanistically explained). Discrete local-gradient wall STANDS (Exp 199-237). The only named escape is OUT OF SCOPE for substrate engineering: a consumer with NO energy buffer (immediate resource-coupling, no lag to damp) — an AGENT-ARCHITECTURE redesign; re-open on a word. WHAT THE HARNESS DELIVERED: a reusable certification instrument (ecology/evolvability/stability.py + cert_run.py) that caught four successive instrument artifacts before they could be published as escapes. META-LESSON: "is a substrate posable for an evolvability test?" is remarkably hard to instrument; rigorous controller-applied gates (not relayed PASS claims) caught each artifact.
