...
**STATUS.** state: active · latest: Exp 241 — candidate escape DOWNGRADED, no stable ESS, invasion was demographic-instability-entangled (both faster AND slower mutants invade 8/8 by explosion at resident equilibrium); the non-saturating per-capita benefit is real (+43% at fixed density); the evolvability claim is not cleanly establishable on this substrate · next-falsifiable: a substrate with proper density regulation giving stable equilibria across a RANGE of speeds (so a clean ESS/invasion test is even possible) — OR close the chapter
