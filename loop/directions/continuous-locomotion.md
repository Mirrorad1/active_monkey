...
**Substrate-engineering arc close (Exp 243–247).** The toy continuous substrate is WEAKLY-POSABLE (stable A-regulated equilibrium; real-but-weak ~5–13% density-dependent competition) but DEMOGRAPHIC STABILITY and STRONG RESOURCE COMPETITION are MUTUALLY EXCLUSIVE on it. Unifying trade-off (Exp 246 capstone): strong competition requires a resource-limited population (per-capita intake ~ cost), but resource-limited dynamics OSCILLATE (lagged energy-buffer feedback, Exp 242). Mechanism A damps oscillations by capping N below resource-limitation → stable, but competition stays weak. Six levers all NEGATIVE: founder, A-strength, regen, bump geometry (Exp 243–245), moving patch (Exp 246), energy buffer (Exp 247). Exp 247 tested the Exp-246 named escape (shrink energy_capacity toward threshold 4.2): oscillation is NOT buffer-driven (CV flat across cap 10→4.8 A-OFF); A is the direct cause. A-OFF: resource-limited (n_eq ~173-183), corr -0.91, drop 15-18%, oscillatory. A-ON: stable (CV ~0.09-0.10), competition weak (~4-8%). The certification harness (ecology/evolvability/stability.py + cert_run.py) caught four instrument artifacts before any escape was published. Only remaining exit is an agent-architecture redesign (NO energy state at all, not just a low cap); weakened premise (oscillation survived near-zero buffer); re-open on a word. Full detail: EXPERIMENTS.md Exp 243–247.

**STATUS.** state: closed-negative · closed: Exp 247 (substrate-engineering arc; six levers exhausted, trade-off shown A-driven) · reusable: ecology/evolvability/stability.py + cert_run.py (oscillation detector + non-degeneracy audit) · why: STABILITY vs STRONG COMPETITION mutually exclusive on this substrate; trade-off is A-driven not buffer-driven; discrete local-gradient wall stands · next: re-open only on a word — agent-architecture redesign (instantaneous-intake, no energy state)
