...
**STATUS.** state: closed-positive (M4a milestone reached; M4b autopilot HARNESS built + validated, no real run yet). latest: M4b harness — active_loop/affect_pr_loop.py + run_affect_loop.py: PR-style loop over MUTABLE affect_spec.py scored by the FROZEN eval/affect_score.py (higher=better); frozen-guard protects the scorer; 5 stub-scored keep/revert tests pass (infra, not a numbered exp). Prior: Exp 222 (1e) shipped converse.py + the FROZEN constant-UNFAKEABLE scorer; Exp 221 = short-session learning boundary; ledger docs/research/m4a-affective-dyad-chapter.md. next: a REAL bounded M4b run (Claude proposer + ~10-min scorer/candidate, ~20min/iter, nested claude -p) — needs a human go + an isolated checkout (cron-safety); alt: a LEARNING-side lever for the short-session wall.
