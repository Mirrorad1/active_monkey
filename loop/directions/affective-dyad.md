...
**STATUS.** state: closed-positive (M4a milestone reached; M4b autopilot RUNS END-TO-END but is critic-gated). latest: Exp 224 — M4b clean re-run (--real x2, AFFECT proposer), NEGATIVE / NEW INSIGHT (blind-verified): after fixing 4 instrument issues the autopilot completes a clean run (rc=0), but at N=2 finds NO improvement — CRITIC-GATED: both proposer prior-changes were rejected by the critic BEFORE the FROZEN scorer measured them (0 merges, best held 0.4225). Improvability UNTESTED by the scorer; the binding gate is the critic. Prior: Exp 223 (shakedown); Exp 222 (1e) converse.py + FROZEN scorer; Exp 221 short-session boundary. next (human word): (a) cleaner re-run (fix journal + capture critic_reason + calibrate the critic); (b) accept (M4a milestone stands); (c) the short-session LEARNING lever.
