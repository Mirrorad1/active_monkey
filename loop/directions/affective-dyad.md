...
**STATUS.** state: active (M4a; realistic regime NOT yet reached — a wall + a metric correction) · latest: Exp 218 — ratchet, PARTIAL_RATCHET / NEW INSIGHT (blind-verified): optimism does NOT rescue the realistic corner (γ1/K4/100t = 0/8, DEGRADES); anchor 7/8 reproduces Exp 217. Real blockers = SHORT SESSION + LOW PRECISION, NOT aliasing (γ4 8/8). KEY catch: the ignition metric under-discriminates — last≥0.30 < the 0.333 constant-response ceiling, so K4's 6/8 is a constant-ceiling artifact (aliasing-rescue WITHDRAWN). Self-healed: constant_response_ceiling + test · depends-on: affect_spec/affect_agent + tests · reusable: yes · why: maps break-points + tightens the metric · next-falsifiable: Exp 219 — 1h: re-run under a discrimination metric (last>1/3) vs session-length + precision.
