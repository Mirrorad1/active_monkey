...
**STATUS.** state: active (M4a; reliable GENUINE discrimination at K=4 confirmed LOAD-BEARING on the long 300t session — short sessions blocked by LEARNING, unrescued by the schedule) · latest: Exp 221 — schedule vs session length (1j), NEGATIVE / NEW INSIGHT (blind-verified): the precision schedule does NOT rescue the short session — short sched cells 0/2/3 of 16 (bar 12), the schedule beats fixed ONLY at 300t (+6) and is slightly WORSE at short lengths (−1/−2/−2); mean_csel rises with length for BOTH conditions ⇒ short sessions block LEARNING (the schedule only optimizes EXPLOITATION). Regression-anchored EXACT to Exp 220 (sched_300=13/16, fixed_300=7/16). This SEPARATES Exp 220's positive from session length: the 300t session is load-bearing · depends-on: affect_spec/affect_agent · reusable: yes · why: clean boundary on the Exp 220 result + confirms the learning blocker · next-falsifiable: CONSULT (a) attack LEARNING at short sessions (lr/optimism/replay, not precision) · (b) accept the long-session milestone + build increment 1e (converse REPL + frozen scorer) · (c) stop/redirect.
