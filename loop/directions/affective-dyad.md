...
**STATUS.** state: active (M4a; FIRST reliable GENUINE discrimination at K=4 — but only at the long 300t session) · latest: Exp 220 — precision schedule, POSITIVE / NEW INSIGHT (blind-verified): annealing γ 1→8 reaches 13/16 GENUINE (honest unfakeable metric) at realistic K=4 — the first reliable genuine result, FIXING the learn-not-exploit decoupling. Beats fixed γ4 (7/16); fixed γ8 WORST (3/16, over-commits). NOT a breakthrough: all cells 300t (the short 100t session — Exp 219's open blocker — untested, so length does much of the work), bar-fragile (csel≥0.67→7/16), N=16 · depends-on: affect_spec/affect_agent · reusable: yes · why: first reliable genuine result · next-falsifiable: Exp 221 — 1j: the schedule at the SHORT session (100/150/200t) — does annealing rescue it or is 300t needed?
