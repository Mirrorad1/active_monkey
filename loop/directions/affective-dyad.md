...
**STATUS.** state: closed-positive (M4a; the "talk to it and learn to feel positive" milestone reached at toy scale on the long session). latest: Exp 222 (1e) — converse.py honest REPL + eval/affect_score.py FROZEN learns-to-positive scorer; MILESTONE_VALIDATED (blind-verified): the learner passes (verdict True: realized POS 0.42>1/3, genuine 6/8, reproduces Exp 220) AND a constant-reply control FAILS (verdict False, genuine 0) = constant-UNFAKEABLE. Long-session boundary (Exp 221) + provided-vs-learned ledger in docs/research/m4a-affective-dyad-chapter.md. next: M4b — PR-style autopilot over MUTABLE affect_spec.py against the FROZEN scorer (raise the genuine number without reward-hacking?); standing alt: a LEARNING-side lever for the short-session wall. Both need a human word.
