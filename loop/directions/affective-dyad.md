...
**STATUS.** state: closed-positive (M4a milestone reached; M4b RESOLVED-POSITIVE + AUTONOMOUS DISCOVERY DEMONSTRATED). latest: Exp 235 (2026-06-18) — real AffectClaudeProposer ran 3 iterations without scripted candidates: iters 0,1 tried pA1-weakening (critic-approved but no improvement); iter 2 autonomously discovered A0 restructuring (encoding the utterance→intent generative relationship) + pA0×20 + flat pA1 → metric 0.4225→1.0, MERGED, POSITIVE (blind-verified). Gaming-ambiguity caveat: CORRECT[c]=c%4 is documented in affect_spec.py line 155 (not in frozen eval/); proposer read it from the mutable file. Key finding: A0 prior specification is the highest-leverage lever (beats C1 NEU=-0.5 from Exp 227 by wide margin). Prior: Exp 227 (scripted find-and-keep, POSITIVE), Exp 225 (critic diagnostic). next (human word): (a) run a CLEAN follow-up removing CORRECT[c]=c%4 from affect_spec.py docstring to test pure metric-feedback discovery; (b) accept the A0-structured affect_spec into main; (c) close arc and move to new direction.
