...
**STATUS.** state: closed-negative · latest: Exp 183 — regulated commitment vs any constant FAILED kill test; defense-as-config documented; chapter graded on human's word · depends-on: meta-calibration-n3 (N3 SUPPORTED), persistent-creature, functional-emergence · reusable: displacement regime (176), rung-2 monitor (180), PC2' vector gate, wrong-control-surface law (181), freeze surface + oracle bound (182), detection-floor + surrender-schedule + commitment-as-config (183), synthesis docs/research/n4-identity-commitment-chapter.md · why: central hypothesis failed pre-registered kill test; re-opens only on a word · next-falsifiable: TBD-human (seed-229/variable-length crack deferred, or untried anchor form)
