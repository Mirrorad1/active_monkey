...
**STATUS.** state: active · latest: Exp 235 (NULL/INVALID — climb_ability behaviorally INERT: the local greedy forager never navigates to the sealed plateau, gate-open access ~1%; bottleneck is the POLICY not the gate; blind-verified) · depends-on: population-ecology (local-gradient wall), evolvability-geometry, ecology/evolvability Preflight · reusable: sealed-plateau terrain + climb_ability LOCOMOTION axis (byte-identical OFF), flat-map + gate-open controls, the gen-0 manipulation-check instrument · why: terrain experiments need a NAVIGATION-capable policy first — the local forager cannot express directed locomotion · next-falsifiable: Exp 236 — add a path-planning policy; FAIL = even with navigation, no geometry makes climb_ability expressible (else proceed to the Gate C gradient).
