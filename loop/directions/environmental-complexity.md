...
**STATUS.** state: active · latest: Exp 236 (NULL/INVALID — a navigation policy does NOT rescue it: honest nav reaches the plateau only under scarcity; closed gate gives ~0 climb marginal (reach/cross/survive trilemma); + an index tie-break gamed the metric, L40; blind-verified) · depends-on: population-ecology (local-gradient wall), evolvability-geometry · reusable: terrain + climb_ability axis + enable_navigation (byte-identical OFF), gate-open/flat-map controls, gen-0 manip-check · why: the gridworld can't jointly satisfy reach+cross+survive for locomotion; escape = PERCEPTION of distant food · next-falsifiable: Exp 237 — a resource-gradient sense (reuse thermosense); FAIL = even with perception no geometry makes climb_ability expressible (then close the gridworld).
