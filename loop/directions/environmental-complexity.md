...
**STATUS.** state: active · latest: Exp 235 predeclared (none logged yet) · depends-on: population-ecology (closed-negative, the local-gradient wall), evolvability-geometry, ecology/evolvability Preflight · reusable: the gated-reachability terrain geometry, climb_ability LOCOMOTION axis, the flat-map + gate-open deflation controls, the L38 reachability manipulation check · why: movement was NEVER an evolvable axis in the wall arc (199–213); a sealed-plateau terrain is the last structurally-distinct escape — a crude mover gets ZERO sealed food, so each ε of climb could unlock a non-saturating chunk · next-falsifiable: Exp 235 Rung 1 — Gate C local gradient (0.05→0.10) <7/8 despite a passing manipulation check = FAIL_LOCAL_GRADIENT, the wall extends to locomotion.
