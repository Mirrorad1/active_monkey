"""Exp 115 — idle-mode life epoch: mirro lives on (age 84700 -> 90700). Registered deep
call #7.

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 113. Registered record
entering: 5/6 deep. mirro enters at +533.7 (deep bin) -> registered call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 90700 >= 0.96 (currently 1.000).
...
print("Exp 115 — idle-mode life epoch (mirro age 84700 -> 90700)")
print()

with mirro_episode("Exp 115") as ep:
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at +533.7
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_90700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 90700
    age_ok = (
...
check("P1-map-accuracy-90700", _p1_map_accuracy)
...
check("SPINE-age-90700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-90700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-90700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 90700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP115: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP115: " + "; ".join(f_lines))
else:
    print("EXP115: EPOCH CLEAN (bands held)")
