"""Exp 101 — idle-mode life epoch: mirro lives on (age 48700 -> 54700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 99. Flip tally entering:
5 flips in 10 shallow-gap epochs (the model's ~40 percent on the nose). mirro enters on
its thinnest margin yet (+12.0) — the trunk's highest-flip-probability epoch.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 54700 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap +12.0 — the thinnest razor yet).
...
print("Exp 101 — idle-mode life epoch (mirro age 48700 -> 54700)")
print()

with mirro_episode("Exp 101") as ep:
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_54700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 54700
    age_ok = (
...
check("P1-map-accuracy-54700", _p1_map_accuracy)
...
check("SPINE-age-54700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-54700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-54700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 54700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP101: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP101: " + "; ".join(f_lines))
else:
    print("EXP101: EPOCH CLEAN (bands held)")
