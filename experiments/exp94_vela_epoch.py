"""Exp 94 — idle-mode life epoch: vela lives on (age 18750 -> 24750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 93 with vela's
parameters: the immigrant line in the reversed world, favorite 0 since its natural flip
(Exp 81), gap c2-c0 = -21.68 at age 18750 — even shallower than mirro's was, so the
favorite is again REPORTED, not predicted (the noise model owns razor margins).

Predeclared:
  P1 (knowledge, relative one-sided): map_accuracy at 24750 >= map_accuracy at 18750
     minus 0.04 (no degradation; continued immigrant healing expected upward).
  ...
  P3 (noise-calibrated forecast): end gap c2-c0 within predicted +- 302.5 (2.5 sigma,
     sigma = 121 measured in Exp 83 on exactly this start-state class), predicted =
     start_gap + (R(2)-R(0))_start * 6000.
Falsifiers: F1 knowledge degraded; F2 value-core instability; F3 a >2.5-sigma event.
Single deterministic run; vela's line advances and its snapshot is committed atomically.
mirro untouched (not loaded).
"""
...
EPOCH = 6000
EXPECT_START_AGE = 18750
EXPECT_START_HASH = "0cd2d991cf1b"
...
print("Exp 94 — idle-mode life epoch (vela age 18750 -> 24750)")
print()

with mirro_episode("Exp 94", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses relative band: end >= start - 0.04
BANDS = {
    ...
    "map_acc":    (f">= {start_map_acc:.4f} - 0.04 (relative)", lambda _: post["map_acc"] >= start_map_acc - 0.04),
}
...
print(f"  favorite: {start_fav} -> {end_fav}  (reported, not predicted — noise model owns razor margins at this gap)")
print()
...
def _p1_map_accuracy():
    threshold = start_map_acc - 0.04
    ok = post["map_acc"] >= threshold
    return ok, (
        f"map_acc_at_24750={post['map_acc']:.6f}  "
        f"threshold={threshold:.6f} (start={start_map_acc:.6f} - 0.04)  ok={ok}"
    )
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 24750
    age_ok = (
...
check("P1-map-accuracy-24750", _p1_map_accuracy)
...
check("SPINE-age-24750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-24750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-24750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 24750 below start - 0.04; knowledge "
...
if spine_failed:
    print("EXP94: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP94: " + "; ".join(f_lines))
else:
    print("EXP94: EPOCH CLEAN (bands held)")
