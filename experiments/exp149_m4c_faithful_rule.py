"""
Exp 149 — continuous-creature rung M4c: the faithful-port ablation (the human's
word "m4c", 2026-06-10).

Card loop/directions/continuous-creature.md (M4). Guardrail: any failed
predeclared property HALTS the migration for explicit human input; a remaining
gap > 0.10 under the faithful rule becomes the DOCUMENTED SUBSTRATE LIMIT (not
chased further).

Hypothesis: Exp 148's structural valence gap (-0.16..-0.19) is the port
infidelity and nothing else. The tabular creature scores predictability at its
MAP point (A_hat at argmax(qs)); Exp 147/148 ported this as the belief-INTEGRATED
predictive, smearing positional uncertainty and footprint leakage into the
entropy. The faithful translation — predictive color entropy evaluated AT the
posterior MEAN (a point: Sigma_eval = 0.01 I, the same evaluation the value-field
policy already uses) — restores the contrast: the continuous reliable-set share
lands within 0.10 of the twin's.

Setup: identical to Exp 148 in every mechanism and constant EXCEPT (a) the
valence weight: weight_t = exp(-H(predictive color distribution at (mu_post,
Sigma_eval = 0.01 I))), computed from the learned (m_k, E[Sigma_k] diag) — one
line changed, everything else byte-for-byte; (b) FRESH seeds 16..23. The
navigation policy, the twin, the mirror arm, all thresholds and definitions are
Exp 148's.

Predictions (TRUE iff all):
- P1a (twin-relative): continuous reliable share >= twin share - 0.10 AND > 0.55,
  in >= 7/8 seeds.
- P1b: Spearman > 0.6 in >= 7/8 seeds.
- P2: favorite-cell occupancy > 4x uniform AND > 2x own phase-A, >= 6/8.
- P3: mirrored favorite on the opposite side, >= 6/8.

Falsifiers (any HALTS): P1a fails in >= 3/8 — the faithful rule STILL gaps; the
shortfall is a genuine substrate property, to be DOCUMENTED as the M4 limit (the
consult then recommends accepting it and proceeding to M5 with the limit logged,
not further variants). P1b/P2/P3 thresholds as before. Three-way rule per
PROTOCOL step 3.
"""
# copied from exp148
...
T_A = 4000
T_B = 2000
# FRESH seeds 16..23 (never run on this protocol)
...
def _predictive_color_logprobs(cp_mu: np.ndarray, cp_Sigma: np.ndarray,
    ...
    Used for place updates (belief-integrated). NOT used for valence in M4c.
    """
    word_mus = [niws[k].expected_mu() for k in range(N_COLORS)]
    word_Sigmas = []
    for k in range(N_COLORS):
...
def run_continuous(
    ...
    Returns a dict with:
      ...
      - mean_weight_reliable: mean valence weight on steps when obs in reliable set
      - mean_weight_noisy: mean valence weight on steps when obs NOT in reliable set
    """
    cp = _init_cp()
    niws = _init_niws()
    ...
    # Diagnostic accumulators for valence weight contrast
    weight_sum_reliable = 0.0
    weight_count_reliable = 0
    weight_sum_noisy = 0.0
    weight_count_noisy = 0
    ...
    for t in range(T_A):
        ...
        # Place update FIRST (to get posterior mean for the faithful valence rule)
        ...
        # This mirrors the tabular twin evaluating at the MAP cell (a point).
        post_mu = cp.mu
        ...
        # Diagnostic: track weight by reliable/noisy
        if obs in reliable_set:
            weight_sum_reliable += weight
            weight_count_reliable += 1
        else:
            weight_sum_noisy += weight
            weight_count_noisy += 1
    ...
    for t in range(T_B):
        ...
        # Diagnostic: track weight by reliable/noisy (phase B)
        if obs in reliable_set:
            weight_sum_reliable += weight
            weight_count_reliable += 1
        else:
    ...
    mean_w_rel = weight_sum_reliable / weight_count_reliable if weight_count_reliable > 0 else 0.0
    mean_w_noisy = weight_sum_noisy / weight_count_noisy if weight_count_noisy > 0 else 0.0
    ...
    return {
        ...
        "occ_a": occ_a,
        "occ_b": occ_b,
        "mean_weight_reliable": mean_w_rel,
        "mean_weight_noisy": mean_w_noisy,
    }
...
def run_tabular(
    ...
    for the twin-relative P1a calibration in Exp 148/149).
    """
...
def main() -> None:
    ...
    print("=" * 80)
    print("Exp 149 — continuous-creature rung M4c: the faithful-port ablation")
    print("=" * 80)
    print()
    ...
    for idx, seed in enumerate(SEEDS):
        ...
        # Diagnostic: mean valence weight on reliable vs noisy
        w_rel = rp["mean_weight_reliable"]
        w_noisy = rp["mean_weight_noisy"]
        ...
        per_seed_data.append({
            ...
            "mean_weight_reliable": float(w_rel),
            "mean_weight_noisy": float(w_noisy),
        })
    ...
    # Diagnostic summary: weight contrast
    print(f"\nValence weight contrast (faithful rule diagnostic):")
    for d in per_seed_data:
        ratio = d['mean_weight_reliable'] / d['mean_weight_noisy'] if d['mean_weight_noisy'] > 0 else float('inf')
        print(f"    seed={d['seed']}: W_reliable={d['mean_weight_reliable']:.4f}  "
              f"W_noisy={d['mean_weight_noisy']:.4f}  ratio={ratio:.3f}")
    ...
    # Falsifiers per spec (M4c):
    # P1a fails in >= 3/8 -> MIGRATION HALT (faithful rule STILL gaps; document as M4 substrate limit)
    ...
    if p1a_failures >= 3:
        halt_triggers.append(
            f"P1a FAILED in {p1a_failures}/8 seeds — the faithful rule STILL gaps; "
            f"the shortfall is a genuine substrate property, DOCUMENTED as M4 substrate limit; "
            f"consult recommends accepting and proceeding to M5 with limit logged"
        )
    if p1b_failures >= 3:
        halt_triggers.append(
    ...
    out_path = Path(__file__).parent / "outputs" / "exp149_rows.json"
