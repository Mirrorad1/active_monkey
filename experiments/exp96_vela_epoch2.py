"""Exp 96 — idle-mode life epoch: vela lives on (age 24750 -> 30750).

Declared idle-mode (Exp 92), vela's turn again. Same structure as Exp 94. Flip-record
tally entering this epoch: 4 flips in 5 shallow-gap epochs (Exp 76, 81, 93, 94 flipped;
Exp 95 held) vs the noise model's ~40 percent per epoch — small-n, tracked.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 30750 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap +41.2 — shallow).
...
print("Exp 96 — idle-mode life epoch (vela age 24750 -> 30750)")
print()

with mirro_episode("Exp 96", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 30750 >= 0.96
BANDS = {
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_30750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 30750
    age_ok = (
...
check("P1-map-accuracy-30750", _p1_map_accuracy)
...
check("SPINE-age-30750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-30750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-30750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 30750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP96: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP96: " + "; ".join(f_lines))
else:
    print("EXP96: EPOCH CLEAN (bands held)")
