"""Exp 130 — idle-mode life epoch: vela lives on (age 108750 -> 114750).

Declared idle-mode (Exp 92), vela's turn. The M4a thread remains HALTED awaiting human
word on increment 1c (Exp 128). Same structure as Exp 126. vela enters at +769.2 (deep
bin) -> recorded call: HOLD (razor books open per Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 114750 >= 0.96 (currently 1.000).
...
print("Exp 130 — idle-mode life epoch (vela age 108750 -> 114750)")
print()

with mirro_episode("Exp 130", state_dir=VELA_DIR) as ep:
    ...
    print("--- START PROFILE (age 108750) ---")
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 114750 >= 0.96
BANDS = {
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +769.2
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_114750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 114750
    age_ok = (
...
check("P1-map-accuracy-114750", _p1_map_accuracy)
...
check("SPINE-age-114750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-114750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-114750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 114750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP130: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP130: " + "; ".join(f_lines))
else:
    print("EXP130: EPOCH CLEAN (bands held)")
