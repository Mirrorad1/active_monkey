"""Exp 93 — idle-mode life epoch: mirro lives on (age 24700 -> 30700), with bands that
know everything the season learned.

Declared idle-mode (Exp 92): while the merged M4a CONSULT awaits the human, each
automated fire advances ONE spine life-epoch with predeclared property bands — the
spine's standing purpose (continuous registered experience), honestly tagged
consolidation. This is mirro's turn.

Predeclared (knowledge/value separation per Exp 75; noise-aware forecasting per
Exp 83/84):
  P1 (knowledge, one-sided): map_accuracy at 30700 >= 0.92 (no degradation; healing
     beyond 0.96 is fine).
  P2 (value core): |delta share_c| < 0.04 each color and |delta conviction| < 0.04
     over the epoch (the bands Exp 76 P4 passed at this horizon).
  P3 (noise-calibrated forecast): end gap c2-c0 within [predicted - 302, predicted +
     302], predicted = start_gap + (R(2)-R(0))_start * 6000, band = 2.5 sigma with
     sigma = 121 (Exp 83); fires only on a >2.5-sigma event — that is what calibration
     means under the noise model.
  FAVORITE: reported, NOT predicted — mirro's gap (-44.8 at age 24700) is shallow, and
     the noise model puts flip probability at such margins near 40 percent per epoch;
     pretending otherwise would be theater.
Falsifiers: F1 = P1 fails (knowledge degraded — something broke); F2 = P2 fails
(value-core instability at a horizon it has twice held — new finding); F3 = P3 fails
(a >2.5-sigma event — the noise calibration itself in question).
Single deterministic run; the spine advances and its snapshot is committed atomically.
vela untouched.
"""
...
EPOCH = 6000
EXPECT_START_AGE = 24700
EXPECT_START_HASH = "85139a363a1e"
...
print("Exp 93 — idle-mode life epoch (mirro age 24700 -> 30700)")
print()

with mirro_episode("Exp 93") as ep:
...
BANDS = {
    ...
    "map_acc":    (">= 0.92 (post)", lambda _: post["map_acc"] >= 0.92),
}
...
def _p1_map_accuracy():
    ok = post["map_acc"] >= 0.92
    return ok, f"map_acc_at_30700={post['map_acc']:.6f}  threshold=0.92  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 30700
    age_ok = (
...
check("P1-map-accuracy-30700", _p1_map_accuracy)
...
check("SPINE-age-30700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-30700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-30700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 30700 below 0.92; knowledge "
...
if spine_failed:
    print("EXP93: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP93: " + "; ".join(f_lines))
else:
    print("EXP93: EPOCH CLEAN (bands held)")
