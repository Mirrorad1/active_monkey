"""Exp 107 — idle-mode life epoch: vela lives on (age 54750 -> 60750) — and the
prospective depth-law test is REGISTERED.

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 105, plus a registration:
the accumulated depth record (15 epochs) shows razors flipping and depth holding, but any
binned analysis of PAST epochs would choose its threshold after seeing the data. Per the
out-of-sample rule (VALIDATION.md, after Exp 70), the test is registered NOW and
evaluated prospectively: REGISTERED — entering |gap| < 60 predicts flip with p >= 0.40;
entering |gap| >= 120 predicts hold with flip-p <= 0.10; epochs with 60 <= |gap| < 120
are unbinned. Evaluation: after >= 10 further idle epochs (around Exp 117), score the
registered bins; the depth law is confirmed if the deep bin shows <= 1 flip in >= 5
deep entries AND the razor bin shows >= 2 flips in >= 4 razor entries (else the law
fails or the sample is declared insufficient, honestly).

This epoch's registered call: vela enters at -139.8 (deep bin) -> HOLD predicted (the
first registered favorite call of idle-mode; P4 records the outcome, evaluation deferred
to the registered test).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 60750 >= 0.96 (currently 1.000).
  ...
  P4 (registered depth call, recorded not checked): predicted HOLD; outcome recorded
     for the Exp ~117 evaluation.
...
print("Exp 107 — idle-mode life epoch (vela age 54750 -> 60750)")
print()

with mirro_episode("Exp 107", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 60750 >= 0.96
BANDS = {
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at -139.8
held_as_called = end_fav == start_fav
p4_outcome = "HELD as called" if held_as_called else "FLIPPED against the call"
print(f"  favorite: {start_fav} -> {end_fav}  (REGISTERED depth call: HOLD — deep bin |gap| >= 120)")
print(f"  P4 registered-call outcome: {p4_outcome} (recorded for the Exp ~117 evaluation)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_60750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 60750
    age_ok = (
...
check("P1-map-accuracy-60750", _p1_map_accuracy)
...
check("SPINE-age-60750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-60750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-60750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 60750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP107: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP107: " + "; ".join(f_lines))
else:
    print("EXP107: EPOCH CLEAN (bands held)")
