"""Exp 179 — N4 rung 2 attempt 3: the identity monitor with an OCCUPANCY-MATCHED scramble control.

Two-arm design on fresh seeds 194-201 (Exp 170 same-schedule binding):
  ...
  Arm B (scramble, generic-surprise control): identical burst windows, but each
    ...
    cell-occupancy histogram measured over steps 5000-6000 of its own session
    (immediately before the first burst; the walk's occupancy is stationary so
    one histogram serves all three bursts — DECLARED), rng 175_000+seed,
    qs reset uniform over ALL cells.
...
PRE-REGISTRATION: predeclared in the Exp 178 entry's Next line and the
identity-n4 card STATUS (committed dc78dd9 BEFORE any new data); PC3b, P2,
P3/F3, P4, D5, and the decision rule retained VERBATIM from the Exp 178 card;
FRESH seeds 194-201; smoke seed 194; prediction: PC3b passes (TV ~0.01) and P3
decides rung-2 vs rung-1.5.
...
SEEDS = list(range(194, 202))   # 8 fresh forks (178 used 186-193)
...
def run_fork(
    ...
    arm='scramble': bursts at the SAME windows, relocation target each burst step
        is drawn from the fork's OWN empirical cell-occupancy histogram measured
        over steps 5000-6000 of its own session (immediately before the first burst;
        the walk's occupancy is stationary so one histogram serves all three bursts
        — DECLARED), rng 175000+seed, qs reset to uniform over ALL cells.
    ...
    fork_name = f"exp179{arm_tag}_s{fork_seed}"
    ...
    # Scramble arm: occupancy histogram computed at step 6000 (steps 5000-6000)
        ...
        for _step_in_chunk in range(CHUNK_SIZE):
            ...
            if in_burst:
                if arm == "identity":
                    ...
                    # Scramble: relocate via fork's own occupancy histogram (steps 5000-6000)
            ...
            # Scramble arm: compute occupancy histogram at end of step 5999
            # (i.e., when global_step == 6000 right after the increment)
            if arm == "scramble" and global_step == 6000 and occ_probs is None:
                occ_hist = np.bincount(
                    true_pos_arr[5000:6000], minlength=n_cells
                ).astype(float)
                if occ_hist.sum() == 0:
                    # Guard: fall back to uniform (cannot happen in practice)
                    occ_probs = np.ones(n_cells) / n_cells
                else:
                    occ_probs = occ_hist / occ_hist.sum()
...
def write_json_rows(
    ...
    with out_path.open("w") as fh:
        ...
        # PC2' rows
        for row in pc2p_rows:
            fh.write(json.dumps({"exp": 179, "phase": "pc2p", **row}) + "\n")

        # PC3b rows
        for row in pc3b_rows:
            fh.write(json.dumps({"exp": 179, "phase": "pc3b", **row}) + "\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Exp 179 — N4 rung 2 attempt 3: occupancy-matched scramble"
    )
    parser.add_argument("--smoke", action="store_true",
                        help="Smoke run: seed [194] only (both arms), no verdict")
    args = parser.parse_args()

    seeds = [194] if args.smoke else SEEDS

    print("=" * 80)
    print("Exp 179 — N4 rung 2 attempt 3: occupancy-matched scramble")
    print("=" * 80)
    print()
    ...
    if not gate_pass:
        ...
        out_rows = Path(__file__).parent / "outputs" / "exp179_rows.json"
    ...
    verdict_path = Path(__file__).parent / "outputs" / "exp179_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp179",
        arms=arms,
        ...
        notes=(
            "N4 rung 2 attempt 3. Occupancy-matched scramble control (fork's own "
            "5000-6000 cell-occupancy histogram) — value-neutral by construction; "
            "PC3b, P2, P3/F3, P4, D5 and decision rule verbatim from the Exp 178 "
            "pre-registered card. Forks of mirro only."
        ),
    )
