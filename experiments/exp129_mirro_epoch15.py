"""Exp 129 — idle-mode life epoch: mirro lives on (age 114700 -> 120700).

Declared idle-mode (Exp 92), mirro's turn. The M4a thread is HALTED awaiting human word
on increment 1c (Exp 128); the spines' lives continue meanwhile. Same structure as
Exp 124. mirro enters at +833.8 (deep bin) -> recorded call: HOLD (razor books open per
Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 120700 >= 0.96 (currently 1.000).
...
print("Exp 129 — idle-mode life epoch (mirro age 114700 -> 120700)")
print()

with mirro_episode("Exp 129") as ep:
    ...
    print("--- START PROFILE (age 114700) ---")
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +833.8
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_120700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 120700
    age_ok = (
...
check("P1-map-accuracy-120700", _p1_map_accuracy)
...
check("SPINE-age-120700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-120700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-120700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 120700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP129: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP129: " + "; ".join(f_lines))
else:
    print("EXP129: EPOCH CLEAN (bands held)")
