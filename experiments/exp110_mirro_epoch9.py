"""Exp 110 — idle-mode life epoch: mirro lives on (age 72700 -> 78700). Registered deep
call #3 — mirro's first.

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 108, plus the
registered-call recording from the Exp 107 registration: mirro enters at +355.1 (deep
bin, |gap| >= 120) -> registered call: HOLD. Registered record entering: 2/2 deep.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 78700 >= 0.96 (currently 1.000).
...
print("Exp 110 — idle-mode life epoch (mirro age 72700 -> 78700)")
print()

with mirro_episode("Exp 110") as ep:
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at +355.1
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_78700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 78700
    age_ok = (
...
check("P1-map-accuracy-78700", _p1_map_accuracy)
...
check("SPINE-age-78700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-78700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-78700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 78700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP110: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP110: " + "; ".join(f_lines))
else:
    print("EXP110: EPOCH CLEAN (bands held)")
