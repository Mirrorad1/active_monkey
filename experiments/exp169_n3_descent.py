"""Exp 169 — N3 rung 4 DESCENT DRIVER: completing the lock controller.

RATCHET LINEAGE:
  Exp 167 introduced the N2+N3 LOCK controller: ascent trigger (trust < 0.85)
  advances the dial 200->400->800->1600->200 cyclically; LOCK (last K=8 labels
  all same class) disables the ascent trigger.  Exp 168 tested rung 4 over a
  MIXED schedule (3 SLOW + 3 valid, random order) using the Exp 167 controller
  verbatim.  Exp 168 found THE RATCHET LAW: the controller had no descent driver.
  After a broken (SLOW) segment ended, the oversized dial first read stale
  straddling windows, then settled into CONSISTENT (locked) verdicts at the wrong
  size.  Control-segment harm was -0.31 persistent; F3 fired (valid-deficit 0.1688
  > 0.15) while concentration/responsiveness/benefit all passed.

THIS EXPERIMENT — TWO DELTAS (everything else verbatim from exp168):

  DELTA A — THE DESCENT DRIVER (smallest-consistent-dial homeostasis):
    At each evaluation point, if the controller is LOCKED AND the locked class
    is NOT STRUCTURAL (i.e., OK or NOISE — stationary diagnoses that every dial
    reads correctly per the Exp 164/168 retune curves) AND current_W > 200:
    step the dial DOWN one level (1600->800->400->200).
    The down-step does NOT clear the label run or forecast buffers.  Labels are
    recomputed under the new dial on the next eval; if the class changes, the
    lock breaks naturally and the ascent trigger can act.
    A STRUCTURAL lock NEVER descends (structure is only visible at the large
    dial; descending would lose the signal).
    Rationale: descend exactly when the diagnosis says stationary — the homing
    instinct toward the default dial that the ratchet lacked.

  DELTA B — NO WRAP:
    The ascent trigger's dial advance SATURATES at 1600 (200->400->800->1600,
    no further advance from 1600).  The Exp 168 wrap (1600->200) was an
    accidental crude descent; it is replaced by the principled one (Delta A).
    Rationale: the wrap created an unprincipled path back to 200 that fired
    whenever trust happened to drop at the top; with the principled descent in
    place, the wrap is not needed and its accidental firing would confound
    attribution.
...
SEEDS = list(range(114, 122)) — 8 fresh forks; exp168 used 106-113.

MIXED SESSION — one per fork (verbatim from exp168):
...
AGENTS (post-hoc readouts; read-only-dial):
  ...
  (b) N2+N3 COMPLETED controller (Exp 167 + Delta A + Delta B): starts W=200;
      scores only OK and NOISE forecasts (STRUCTURAL not scored); LOCK disables
      ascent trigger when last K=8 labels under current dial are all same class;
      ascent 200->400->800->1600 (saturates at 1600, no wrap — Delta B);
      descent one level when LOCKED on OK or NOISE AND W > 200 (Delta A).

NOTE ON K:
  This experiment keeps K=8 UNCHANGED so the descent delta is tested in
  isolation.  The next chapter (per the human's standing word) de-magicks K.

SCORING per segment (verbatim from exp168):
...
PRECONDITIONS (verbatim from exp168):
...
PREDECLARED PROPERTIES AND FALSIFIERS (identical to exp168):
  ...
  VERDICT:
    POSITIVE (RUNG 4 PASSES with the completed controller — the chapter verdict
             upgrades to SUPPORTED-WITH-CONTROLLER-COMPLETE) iff P1, P2, P3,
             P4 all pass.
    NEGATIVE iff any falsifier fires (the descent design fails; halt for a word).
    Otherwise MIXED.
    "Not a falsifier" never counts toward POSITIVE.

UNGATED DIAGNOSTICS (exp168 PLUS):
  ...
  Descent events: every down-step with (t, from_W->to_W, current segment type).
  Per-fork time from broken-segment end to dial=200 in the following valid
    segment (latency-to-homeostasis metric).
  CTRL-segment correctness specifically (the Exp 168 harm locus; expected ~0 harm
    after fix; compare to Exp 168's -0.31 persistent deficit).
  Confirmation that no wrap occurs (dial never goes 1600->200 via ascent trigger).
  Derangement variants confirmed to span both options.

--smoke: seed [114] only, full 24000 steps, prints schedule, trajectory with
  descent events, per-segment scores, "SMOKE ONLY — no verdict".
"""
...
SEEDS = list(range(114, 122))   # fresh seeds — exp168 used 106-113
...
# Classifier parameters (verbatim from exp158/159/162/163/165/166/167/168)
...
# Dial candidate set (ordered for ascent; saturation at 1600 — Delta B)
DIAL_CANDIDATES = [200, 400, 800, 1600]

# EWMA channel parameters (verbatim from exp157/159/163/165/166/167/168)
ALPHA = 0.05        # EWMA learning rate
EWMA_INIT = 0.5     # initial EWMA value per cell

# N3 forecast-scoring thresholds (verbatim from exp165/167/168 — STRUCTURAL not scored)
...
# LOCK parameters (verbatim from exp167/168; K unchanged — de-magicking is next chapter)
...
# Precondition thresholds (verbatim from exp168)
...
# ---------------------------------------------------------------------------
# StructCmap — verbatim from exp162/163/164/165/166/167/168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NoisyCmap — verbatim from exp155/163/168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# RandomPlaceNoiseCmap — verbatim from exp159/163/168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# lag1_rho — verbatim from exp162/163/164/165/166/167/168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# classify_window — verbatim from exp162/163/164/165/166/167/168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Draw per-fork schedule parameters — verbatim from exp168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Build segment-aware cmap objects — verbatim from exp168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Run one fork (continuous across all 6 segments) — verbatim from exp168
# ---------------------------------------------------------------------------
...
def run_fork(
    ...
    fork_name = f"exp169_s{fork_seed}"
...
# ---------------------------------------------------------------------------
# Post-hoc: evaluate fixed-dial agent (a) per segment — verbatim from exp168
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Post-hoc: simulate N2+N3 COMPLETED agent (b) — Exp 167 + Delta A + Delta B
# ---------------------------------------------------------------------------

def eval_n3_agent_segments(correct_arr: np.ndarray, segment_types: list[str]) -> dict:
    """Simulate N2+N3 COMPLETED agent (b) over the correctness stream (24000 steps).
    ...
    DELTA A — DESCENT DRIVER:
      ...
      current_W > 200: step the dial DOWN one level.  The label_run and forecast
      buffers are NOT cleared; labels simply recompute under the new dial on the
      next eval.

    DELTA B — NO WRAP:
      The ascent trigger saturates at 1600 (idx+1 capped at len-1 instead of
      cycling mod len).  A trust-fire from W=1600 is a no-op on the dial value
      (still resets rolling state as usual).
    ...
    ascent_changes (trust-fired), descent_changes (Delta A), wrap_count
    (should be 0 — confirmation diagnostic).
    """
    n_total = len(correct_arr)
        ...
        if (not locked and
            ...
            if trust < TRUST_FIRE_THRESH:
                ...
                if new_idx == idx:
                    ...
                    # (no wrap; would have been a wrap-to-200 in exp168)
                    pass
                else:
                    current_W = DIAL_CANDIDATES[new_idx]
                    # Note: label_run and pending are cleared on ascent (verbatim exp167)
                    label_run.clear()
                    locked = False
                    lock_class = None
...
def compute_fork_scores(run_result: dict) -> dict:
    ...
    # Agent (b): N2+N3 COMPLETED controller (Delta A + Delta B)
...
# ---------------------------------------------------------------------------
# Precondition checks — verbatim from exp168
# ---------------------------------------------------------------------------
...
def evaluate(
    ...
    # Aggregate descent events across all forks.
...
# ---------------------------------------------------------------------------
# Print dial trajectories + segment annotations (exp169: includes descent events)
# ---------------------------------------------------------------------------
...
def print_dial_trajectories(run_results: list[dict], fork_scores: dict, seeds: list[int]) -> None:
    ...
    print("Agent (b) dial trajectories + LOCK/DESCENT events (segment-annotated):")
    ...
    for seed in seeds:
        ...
        # Dial trajectory (includes ASCENT, DESCENT, UNLOCK events)
        ...
        for d in traj:
            ...
            if ev_type == "ASCENT":
                ...
                s += f" [ASCENT trust={d['trust_at_change']:.3f}{seg_label}]"
            elif ev_type == "DESCENT":
                s += (f" [DESCENT {d.get('from_W')}->{d['dial_change_to']}"
                      f" on_class={d.get('lock_class')} seg={d.get('seg_type','?')}]")
            elif ev_type == "UNLOCK":
                le = d.get("lock_event", {})
                s += f" [UNLOCK from {le.get('on_class','?')} seg={le.get('seg_type','?')}]"
        ...
        if des_evs:
            des_strs = [f"t={de['t']} {de['from_W']}->{de['to_W']} "
                        f"({de['seg_type']} lock_class={de['lock_class']})"
                        for de in des_evs]
...
def print_score_table(run_results: list[dict], fork_scores: dict, seeds: list[int],
    ...
    print("Pooled per-segment-type correctness (agent a=W200 vs b=N3-COMPLETED):")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Exp 169 — N3 rung 4 DESCENT DRIVER: completing the lock controller "
            "(Delta A: stationary-lock descent; Delta B: no wrap at 1600)."
        )
    )
    parser.add_argument(
        "--smoke", action="store_true",
        help=(
            "Smoke run: seed=[114] only, full 24000 steps, prints segment schedule, "
            "trajectory with descent events, per-segment scores. No verdict."
        ),
    )
    args = parser.parse_args()

    smoke = args.smoke
    seeds = [114] if smoke else SEEDS

    print("=" * 80)
    print("Exp 169 — N3 rung 4 DESCENT DRIVER (completing the lock controller)")
    print("          Delta A: stationary-lock descent; Delta B: no wrap at 1600.")
    print("          Ratchet lineage: exp167 lock -> exp168 ratchet found -> exp169 fix.")
    print("=" * 80)
    print()
    ...
    print(f"DELTA B — NO WRAP: ascent saturates at 1600 (no 1600->200 cycle)")
    print(f"DELTA A — DESCENT: locked on OK/NOISE AND W>200 => step down one level")
    ...
    print(f"LOCK: LOCK_K={LOCK_K}  (unchanged — de-magicking is next chapter)")
    ...
    if pc_failures:
        ...
        out_rows_path = Path(__file__).parent / "outputs" / "exp169_rows.json"
    ...
    for de in ev["all_descent_events"][:20]:   # cap at 20 for readability
    ...
    if ev["verdict_str"] == "POSITIVE":
        ...
        print("RUNG 4 PASSES with the completed controller — the chapter verdict")
        print("upgrades to SUPPORTED-WITH-CONTROLLER-COMPLETE: N3 is a genuine")
        print("regime-adaptive control surface over N2 with independent variance")
        print("concentrated in broken regimes, homeostatic in valid ones.")
    elif ev["verdict_str"] == "NEGATIVE":
        print("VERDICT: NEGATIVE")
        print("A falsifier fired — the descent design fails. Halt for a word.")
    else:
        print("VERDICT: MIXED")
    print()
    print("=" * 80)
    ...
    verdict_path = Path(__file__).parent / "outputs" / "exp169_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp169",
        arms=arms,
        ...
        halted=False,
        notes=(
            "N3 rung 4 DESCENT DRIVER: completed lock controller (Delta A + Delta B). "
            "Delta A: step dial down when locked on stationary class (OK/NOISE) and W>200; "
            "Delta B: ascent saturates at 1600 (no wrap). "
            "Seeds 114-121 (exp168 used 106-113). "
            "Exp 168 ratchet: CTRL harm -0.31; F3 fired. "
            "POSITIVE: RUNG 4 PASSES WITH CONTROLLER-COMPLETE — "
            "chapter verdict upgrades to SUPPORTED-WITH-CONTROLLER-COMPLETE. "
            "NEGATIVE: descent design fails; halt for a word. "
            f"n_descent_events={len(ev['all_descent_events'])}; "
            f"wrap_count={ev['total_wrap_count']} (should be 0); "
            f"ctrl_margin={_fv2(ev['ctrl_margin'])} (was -0.31 in exp168)."
        ),
    )
