"""Exp 99 — idle-mode life epoch: mirro lives on (age 42700 -> 48700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 97. Flip tally entering:
5 flips in 8 shallow-gap epochs (the model's ~40 percent looking good). mirro's margin
is back in razor territory (+30.3).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 48700 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap +30.3 — razor).
...
print("Exp 99 — idle-mode life epoch (mirro age 42700 -> 48700)")
print()

with mirro_episode("Exp 99") as ep:
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_48700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 48700
    age_ok = (
...
check("P1-map-accuracy-48700", _p1_map_accuracy)
...
check("SPINE-age-48700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-48700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-48700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 48700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP99: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP99: " + "; ".join(f_lines))
else:
    print("EXP99: EPOCH CLEAN (bands held)")
