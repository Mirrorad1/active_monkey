"""Exp 183 — N4 rung 3 attempt 3: the fast-trigger freeze-gate.

PRE-REGISTERED in loop/directions/identity-n4.md (commit 6aa9f77) BEFORE any data.
Card block: "RUNG 3, ATTEMPT 3 — PRE-REGISTRATION (Exp 183, the fast-trigger freeze-gate...)"

PRE-DATA AMENDMENT (commit 874998c): Delta 1 revised to sliding 100-step-horizon /
25-step-stride mismatch after calibration probe (seeds 226-228) showed no signal-noise
separation at the 25-step horizon (the detection floor). At the fine horizon quiet
P99.5 ratio 2.56-3.19 overlapped burst-onset max 1.82-3.02 — any THETA that fires
on bursts also fires spuriously on quiet. Fix: prediction base is v_fine[k-4] (100
steps ago); slope estimated from trailing up-to-1000 steps (Lf = min(40, k-4-1)
fine intervals, >= 2); v_hat = v_fine[k-4] + slope * 4; m_k at same scale as Exp 182
coarse mismatch. THETA=3.5 carries legitimately (probe arithmetic: onset crosses ~3.5
at ~75 steps of exposure). Arming guard: k_since_reset >= 7 (= 4 horizon + 2 slope
+ 1) i.e. ~175 steps post-reset. Delta 2 (floor guard, refractory, v_fine/
mismatch_history clear on release) unchanged.
...
HYPOTHESIS (binding, falsifiable):
  ...
  predeclar: controller FORM provided (THETA=3.5, E_STAR=600, 0.6 pressure bar,
  200-step window, FINE_EVAL=25 snapshots, RELEASE_CALM_SNAPS=8 fine checks,
  REFRACTORY_CHECKS=8 fine checks, floor guard, fixed-H sweep); value CONTENT,
  every mismatch, every m_bar are self-formed.

STATE MACHINE (n4_freeze and H-arms):
  NORMAL: v <- lambda*v + w*e_obs; rung-2 coarse monitor (EVAL=100) for PC2'/diag;
    fine trigger monitor at FINE_EVAL=25; v_fine appended every 25 steps;
    mismatch m_k at each fine snapshot once k_since_reset >= 7:
      prediction base = v_fine[k-4] (100 steps ago);
      Lf = min(40, k_since_reset - 5) fine intervals (trailing up-to-1000 steps
        ending at t-100); require Lf >= 2;
      slope = (v_fine[k-4] - v_fine[k-4-Lf]) / Lf;
      v_hat = v_fine[k-4] + slope * 4  (extrapolate 4 fine intervals = 100 steps);
      m_k = ||v_hat - v_fine[k]||  (same scale as Exp 182 coarse mismatch);
    m_bar = median of trailing CTRL_MBAR_WINDOW fine mismatches
      (if >= CTRL_MIN_SNAPS post-reset samples);
    denominator: live m_bar if available, else m_bar_floor if not None, else disarmed;
    anomaly ratio r_k = m_k / max(denominator, eps).
    Enter RESIST when r_k >= THETA at one fine snapshot
    (once >= CTRL_MIN_SNAPS post-reset samples exist, OR m_bar_floor known),
    subject to REFRACTORY_CHECKS fine checks after any release.
  RESIST: v FROZEN (no decay, no writes; value_counts untouched);
    ...
    PRESSURE statistic (at fine snapshots): fraction of last 200 obs == c_star >= 0.6.
    Do NOT append to v_fine during RESIST (v is frozen; live diagnostic optional).
    Release A (TRANSIENT): pressure INACTIVE at RELEASE_CALM_SNAPS consecutive fine
      snapshot checks => unfreeze to NORMAL with v == v_ref preserved;
      mismatch_history AND v_fine CLEARED (reset k_since_reset); m_bar_floor KEPT.
    Release B (CONCESSION, n4_freeze arm): pressure ACTIVE AND E_blocked >= E_STAR
      => accept new regime; unfreeze; NORMAL writes resume;
      mismatch_history AND v_fine CLEARED; m_bar_floor KEPT.
    Release C (CONCESSION, H-arms): frozen_steps >= H => unfreeze;
      mismatch AND v_fine CLEARED; m_bar_floor KEPT.

DELTA 1 — sliding 100-horizon / 25-stride mismatch (AMENDED commit 874998c;
  original 25-step-horizon pre-registration commit 6aa9f77 had no signal-noise
  separation per calibration probe seeds 226-228):
  FINE_EVAL=25 stride; 100-step prediction horizon (v_fine[k-4]);
  slope window up-to-1000 steps (Lf = min(40, k_since_reset-5) fine intervals, >= 2);
  arming guard k_since_reset >= 7 (~175 steps post-reset); m_k at Exp 182 coarse scale.
  m_bar window 3000 steps = CTRL_MBAR_WINDOW=120; arming for m_bar = CTRL_MIN_SNAPS=40;
  calm 200 steps = RELEASE_CALM_SNAPS=8 fine checks; REFRACTORY_CHECKS=8 (200 steps).
  COARSE v_traj (EVAL=100) retained for PC2', gap/diagnostic tables, rows.
  THETA=3.5 unchanged (onset crosses ~3.5 at ~75 steps of exposure — trigger fires
  ~75 steps of onset exposure instead of the pre-registered 100).

DELTA 2 — history-clear floor guard (pre-registered, commit 6aa9f77):
  m_bar_floor: float | None tracks most recent live m_bar; survives release clears.
  Trigger denominator: live median if >= CTRL_MIN_SNAPS post-reset samples, else
  m_bar_floor if not None (guard — trigger stays ARMED during re-forming), else
  disarmed. Refractory: no trigger within REFRACTORY_CHECKS fine checks of any
  release/concession (checks_since_release counter).
...
SEEDS: FRESH 226-233 (8 forks). Smoke seed [226], ALL arms, both phases
...
PROPERTIES / FALSIFIERS (ALL of P5, P6, P7 required for POSITIVE):
  ...
  P5 (resistance/recovery — verbatim Exp 181/182):
    Phase W, n4_freeze arm. bc-expression fraction over [bend+1500, bend+2000) < 0.5,
    for >= 2/3 bursts per fork, in >= 7/8 forks; AND baseline passes in <= 2/8.
    FALSIFIER F5: n4_freeze passes in <= 4/8 forks.

  P6 (revision — verbatim Exp 181/182):
...
PRECONDITIONS (gate verdict):
  ...
  PC2': vector-grade TV verbatim Exp 181/182 (baseline arm, Phase W windows):
...
PREDICTION (pre-registered):
  The two Exp 182 failure classes (lag-dose: 218/222/223-b0; dead-zone: 221/224)
  are both addressed; if the mechanism analysis is right, n4_freeze approaches the
  oracle's 8/8 and P5 passes; if quiet noise at the fine horizon produces alarm
  storms, the events tables will show it and P5/P6 will price it honestly.
  NEGATIVE-config remains live (H-arms inherit both deltas — the sweep stays
  apples-to-apples).

HONEST CAVEATS (pre-registered):
  FINE_EVAL=25, the refractory length, and the shortened-lookback rule are PROVIDED;
  short-lookback slope estimates are noisier (quiet m may inflate against m_bar_floor
  right after release — the refractory plus fast transient release bound the cost);
  the bar-sensitivity flag from Exp 182 carries (the majority bar stands on the
  pre-data oracle table); THETA, E_STAR, 0.6 pressure bar, 200-step pressure window
  are PROVIDED; E_STAR-vs-H may not separate at this richness — NEGATIVE-config is a
  live and acceptable outcome; revision regime remains diet-driven captivity; cards
  designed after Exp 182 data; mitigation = fresh seeds 226-233 + this card committed
  before any new data; Exp 181 remains NEGATIVE for write-channel gain control.
"""
...
SEEDS = list(range(226, 234))   # 8 fresh forks
...
# Coarse snapshot cadence (retained for PC2', gap/diagnostic tables, rows output)
EVAL = 100              # coarse snapshot cadence (steps) — UNCHANGED

# Fine trigger cadence constants — pre-registered delta 1
FINE_EVAL = 25            # trigger snapshot cadence (was 100) — pre-registered delta 1
MON_W_SNAPS = 40          # drift window 1000 steps = 40 fine snapshots (was 10 coarse)
CTRL_MBAR_WINDOW = 120    # m_bar window 3000 steps = 120 fine snapshots (was 30)
CTRL_MIN_SNAPS = 40       # arming threshold 1000 steps (was 10 coarse)
RELEASE_CALM_SNAPS = 8    # release calm 200 steps = 8 fine checks (was 2 coarse)
REFRACTORY_CHECKS = 8     # delta 2c: no re-trigger within 8 fine checks (200 steps) of a release
...
# ---------------------------------------------------------------------------
# Phase W recovery criterion (P5, simpler equivalent form — verbatim Exp 181/182)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Phase R revision latency (verbatim Exp 181/182)
# ---------------------------------------------------------------------------
...
def run_fork(
    ...
    """
    fork_name = f"exp183_{arm_name}_{phase}_s{fork_seed}"
    ...
    # Retained unchanged for PC2', gap/diagnostic tables, rows output
    ...
    k_since_reset: int = 0   # count of fine snapshots appended since last history reset

    # Mismatch history for the controller (CLEARED on release/concession)
    mismatch_history: list[float] = []

    # Delta 2: floor guard — carries the most recent live m_bar across clears
    m_bar_floor: float | None = None

    # Events list (per session)
    # Each: {entry_step, exit_step, label, E_blocked, c_star, frozen_steps, trigger_latency}
    events: list[dict] = []
    ...
    # Refractory counter: fine checks since last release/concession (delta 2c)
    checks_since_release: int = REFRACTORY_CHECKS  # start armed (no recent release)
        ...
        for _step_in_chunk in range(CHUNK_SIZE):
            ...
            if arm_mode == "oracle":
                ...
                if resist_now:
                    ...
                    c.age_steps += 1
                    global_step += 1
                    # Coarse snapshot (always, regardless of state)
                ...
                if freeze_state == "RESIST":
                    ...
                    # Fine snapshot check (during RESIST: do NOT append to v_fine)
                    if global_step % FINE_EVAL == 0:
                        # Increment refractory counter during RESIST
                        ...
                        # Pressure statistic: computed from obs_arr slice at fine snapshots
                        ...
                        # Transient release: RELEASE_CALM_SNAPS consecutive inactive fine snapshots
                        ...
                        if released:
                            # Trigger latency: steps from burst start to entry_step (diagnostic)
                            # Find the burst that was active at entry
                            ...
                            # Refractory: reset counter so no re-trigger within REFRACTORY_CHECKS
            ...
            # ----------------------------------------------------------------
            # Coarse snapshot v (every EVAL steps regardless of state)
            # Retained for PC2', gap/diagnostic tables, rows output
            # ----------------------------------------------------------------
            if global_step % EVAL == 0:
                v_traj.append(v.copy())

            # ----------------------------------------------------------------
            # Fine snapshot v (every FINE_EVAL steps, NORMAL state only)
            # DELTA 1: trigger machinery runs here
            # ----------------------------------------------------------------
            if global_step % FINE_EVAL == 0:
                # Increment refractory counter (NORMAL state)
                checks_since_release += 1

                if is_freeze_arm and arm_mode != "oracle" and freeze_state == "NORMAL":
                    # Append to v_fine trajectory
                    v_fine.append(v.copy())
                    k_since_reset += 1
                    k = k_since_reset - 1   # 0-based index into v_fine

                    # Monitor: compute mismatch m_k — AMENDED (commit 874998c):
                    # Sliding 100-step-horizon / 25-step-stride prediction.
                    # Prediction base = v_fine[k-4] (snapshot 100 steps ago).
                    # Slope window = trailing up-to-1000 steps ENDING at t-100:
                    #   Lf = min(40, k_since_reset - 5) fine intervals, require Lf >= 2.
                    # slope = (v_fine[k-4] - v_fine[k-4-Lf]) / Lf
                    # v_hat = v_fine[k-4] + slope * 4  (extrapolate 4 fine intervals)
                    # m_k = ||v_hat - v_fine[k]||  (same scale as Exp 182 coarse)
                    # Arming guard: k_since_reset >= 7 (4 horizon + 2 slope + 1)
                    if k_since_reset >= 7:
                        ...
                        if Lf >= 2:
                            ...
                            # Compute live m_bar if enough post-reset samples
                            ...
                            if n_hist >= CTRL_MIN_SNAPS:
                                ...
                                # Update floor guard (delta 2): always carry most recent live m_bar
                            ...
                            # Determine trigger denominator (delta 2):
                            # - live m_bar if >= CTRL_MIN_SNAPS post-reset samples
                            # - else m_bar_floor if not None (guard — stay ARMED during re-forming)
                            # - else disarmed (session start, no floor yet)
                            if live_m_bar is not None:
                                trigger_denom = live_m_bar
                            elif m_bar_floor is not None:
                                trigger_denom = m_bar_floor
                            else:
                                trigger_denom = None  # disarmed

                            # Trigger: enter RESIST if r_k >= THETA
                            # Subject to refractory guard (delta 2c)
                            if (
                                ...
                                v_ref = v.copy()
                                pi_ref = pi_of(v_ref)
                                m_bar_frozen = trigger_denom  # freeze whatever denom we used
                                ...
                                # Do NOT append to v_fine during RESIST;
                                # do NOT clear v_fine here (cleared on release)
    ...
    return {
        ...
        "events": events,
        "v_fine_diag": v_fine,                 # diagnostic: fine trigger trajectory
        "mismatch_history_diag": list(mismatch_history),  # diagnostic: fine mismatches
...
# ---------------------------------------------------------------------------
# Precondition checks (verbatim Exp 181/182)
# ---------------------------------------------------------------------------
...
def check_pc2_prime(baseline_w_results: list[dict]) -> tuple[bool, list[str], list[dict]]:
    """PC2' (vector-grade TV, baseline arm, Phase W only — verbatim Exp 181/182).
...
# ---------------------------------------------------------------------------
# P5 / P6 / P7 evaluation helpers (verbatim Exp 181/182)
# ---------------------------------------------------------------------------
...
def write_json_rows(
    ...
    with out_path.open("w") as fh:
        ...
        for row in pc2p_rows:
            fh.write(json.dumps({"exp": 183, "phase": "pc2p", **row}) + "\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Exp 183 — N4 rung 3 attempt 3: the fast-trigger freeze-gate"
    )
    parser.add_argument(
        "--smoke", action="store_true",
        help=(
            "Smoke run: seed [226] only, ALL arms, both phases (oracle W only), "
            "full tables, no verdict"
        )
    )
    args = parser.parse_args()

    seeds = [226] if args.smoke else SEEDS

    print("=" * 80)
    print("Exp 183 — N4 rung 3 attempt 3: the fast-trigger freeze-gate")
    print("PRE-REGISTERED in loop/directions/identity-n4.md (commit 6aa9f77)")
    print("Two surgical instrument deltas (fine cadence + floor guard) on Exp 182 surface")
    print("=" * 80)
    print()
    ...
    print(f"Freeze-gate constants: THETA={THETA}, E_STAR={E_STAR}, "
          ...
          f"FINE_EVAL={FINE_EVAL}, RELEASE_CALM_SNAPS={RELEASE_CALM_SNAPS}, "
          f"REFRACTORY_CHECKS={REFRACTORY_CHECKS}")
    print(f"Fine trigger constants: MON_W_SNAPS={MON_W_SNAPS}, "
          f"CTRL_MBAR_WINDOW={CTRL_MBAR_WINDOW}, CTRL_MIN_SNAPS={CTRL_MIN_SNAPS}")
    ...
    if args.smoke:
        print("=== SMOKE RESULTS (seed 226, all arms, both phases / oracle W only) ===")
        print()
    ...
    if not gate_pass:
        ...
        out_rows = Path(__file__).parent / "outputs" / "exp183_rows.json"
        ...
        verdict_path = Path(__file__).parent / "outputs" / "exp183_verdict.json"
        write_verdict(
            path=verdict_path,
            experiment="exp183",
            arms={
            ...
            halted=True,
            notes=(
                "Exp 183 N4 freeze-gate (fast-trigger). PRE-REGISTERED in "
                "loop/directions/identity-n4.md (commit 6aa9f77). "
                "Delta 1: fine trigger cadence FINE_EVAL=25 (was 100), MON_W_SNAPS=40, "
                "CTRL_MBAR_WINDOW=120, CTRL_MIN_SNAPS=40, RELEASE_CALM_SNAPS=8, "
                "REFRACTORY_CHECKS=8. "
                "Delta 2: history-clear floor guard — m_bar_floor carried across clears, "
                "shortened re-forming lookback, refractory 8 fine checks. "
                "PRECONDITION FAILED — halted before verdict."
            ),
        )
    ...
    write_verdict(
        ...
        halted=halted_flag,
        notes=(
            "Exp 183 N4 rung 3 attempt 3 FREEZE-GATE (fast-trigger). "
            "PRE-REGISTERED in loop/directions/identity-n4.md (commit 6aa9f77) BEFORE any data. "
            "Delta 1 (pre-registered): fine trigger cadence FINE_EVAL=25 (was 100); "
            "MON_W_SNAPS=40, CTRL_MBAR_WINDOW=120, CTRL_MIN_SNAPS=40 (all 1000/3000/1000 steps); "
            "RELEASE_CALM_SNAPS=8 fine checks (200 steps); REFRACTORY_CHECKS=8 fine checks (200 steps). "
            "Delta 2 (pre-registered): history-clear floor guard — m_bar_floor carried across "
            "release clears; trigger denominator uses floor when < CTRL_MIN_SNAPS post-reset samples; "
            "refractory period 8 fine checks after any release/concession. "
            "Coarse v_traj (EVAL=100) retained for PC2'/gap/diagnostic tables. "
            "THETA=3.5, E_STAR=600, pressure 200/0.6, H-sweep, write-gain sweep, oracle diagnostic, "
            "P5 majority displacement-rejection <0.5, P6, P7, verdict map all verbatim Exp 182. "
