"""Exp 108 — idle-mode life epoch: mirro lives on (age 66700 -> 72700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 106. The registered
depth-law test (Exp 107) is live; mirro enters at +115.0 — the UNBINNED middle
(60 <= |gap| < 120), so this epoch contributes no registered call (favorite reported
as always; registered record stands at 1/1 deep).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 72700 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap +115.0 — unbinned middle).
...
print("Exp 108 — idle-mode life epoch (mirro age 66700 -> 72700)")
print()

with mirro_episode("Exp 108") as ep:
...
print(f"  favorite: {start_fav} -> {end_fav}  (reported, not predicted — unbinned middle, start gap +115.0)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_72700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 72700
    age_ok = (
...
check("P1-map-accuracy-72700", _p1_map_accuracy)
...
check("SPINE-age-72700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-72700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-72700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 72700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP108: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP108: " + "; ".join(f_lines))
else:
    print("EXP108: EPOCH CLEAN (bands held)")
