"""Exp 106 — idle-mode life epoch: mirro lives on (age 60700 -> 66700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 104. Flip tally
entering: 6 flips in 14 epochs (mixed depths; binned analysis pending). mirro enters on
a razor (-23.0).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 66700 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap -23.0 — razor).
...
print("Exp 106 — idle-mode life epoch (mirro age 60700 -> 66700)")
print()

with mirro_episode("Exp 106") as ep:
...
print(f"  favorite: {start_fav} -> {end_fav}  (reported, not predicted — razor start gap -23.0)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_66700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 66700
    age_ok = (
...
check("P1-map-accuracy-66700", _p1_map_accuracy)
...
check("SPINE-age-66700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-66700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-66700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 66700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP106: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP106: " + "; ".join(f_lines))
else:
    print("EXP106: EPOCH CLEAN (bands held)")
