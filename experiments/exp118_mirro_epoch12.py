"""Exp 118 — idle-mode life epoch: mirro lives on (age 90700 -> 96700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 115. The Exp 117
evaluation scored the registration's first cycle (deep confirmed-at-limit; razor
insufficient, books open); deep calls continue to be recorded for any second cycle.
mirro enters at +601.1 (deep bin) -> recorded call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 96700 >= 0.96 (currently 1.000).
...
print("Exp 118 — idle-mode life epoch (mirro age 90700 -> 96700)")
print()

with mirro_episode("Exp 118") as ep:
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +601.1
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_96700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 96700
    age_ok = (
...
check("P1-map-accuracy-96700", _p1_map_accuracy)
...
check("SPINE-age-96700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-96700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-96700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 96700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP118: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP118: " + "; ".join(f_lines))
else:
    print("EXP118: EPOCH CLEAN (bands held)")
