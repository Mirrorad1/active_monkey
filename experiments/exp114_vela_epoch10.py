"""Exp 114 — idle-mode life epoch: vela lives on (age 72750 -> 78750). Registered deep
call #6.

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 111. Registered record
entering: 4/5 deep (tolerance spent at Exp 111; deep flip rate measured 0.20 at Exp 112;
the registration stands as written). vela enters at +192.6 (deep bin, post-lurch on the
color-2 side) -> registered call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 78750 >= 0.96 (currently 1.000).
...
print("Exp 114 — idle-mode life epoch (vela age 72750 -> 78750)")
print()

with mirro_episode("Exp 114", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 78750 >= 0.96
BANDS = {
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at +192.6
# (generic: outcome is end favorite vs START favorite, whatever the start favorite is)
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_78750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 78750
    age_ok = (
...
check("P1-map-accuracy-78750", _p1_map_accuracy)
...
check("SPINE-age-78750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-78750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-78750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 78750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP114: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP114: " + "; ".join(f_lines))
else:
    print("EXP114: EPOCH CLEAN (bands held)")
