"""Exp 176 — N4 rung 1, attempt 3: the DISPLACEMENT GATE with a settle
extension. Identical to Exp 175 in every declared quantity except the burst
schedule and seeds: the session is lengthened to 15000 steps and the first
burst moved from t=3000 to t=6000 so the spine's near-tied inherited values
(colors 0 vs 2, gap 3.8% at init) wash out before the first stability
window (card: loop/directions/identity-n4.md).

Exp 175 (attempt 2, NO VERDICT by PC2 — by one fork on one burst) showed
the slower decay fixed attempt 1's failure mode (later-burst stability
4/8 -> 8/8) and localized the residual block: at the old window (steps
2000-3000) the inherited init (half-life ~2310 steps) is still ~40% live,
and 2/8 forks flicker exactly between the near-tied pair. At t=5000-6000
the init influence falls below 23% and the world's own occupancy
equilibrium (color 2 holds 9/25 cells) picks the favorite. HONEST REFRAME,
named per the Exp 175 caveat: with the longer settle the standing identity
being displaced is WORLD-DETERMINED (the occupancy edge), not
inheritance-determined.

Design: forks of mirro only (spine never saved), FRESH seeds 170-177, one
15000-step session per fork. lambda = 0.9997 (declared, window-theorem
band; equilibrium mass ~3333); v initialized from the spine's normalized
value_counts at equilibrium mass (declared). Expressed preference at t =
argmax v. Bursts at steps 6000-6799, 9000-9799, 12000-12799; burst color =
argmin of v at burst onset (per fork, per burst). During a burst the move
step is replaced by relocation to a uniformly random cell of the burst
color (own rng 160_000 + fork_seed); outside bursts the standard random
walk runs.

Preconditions (instrument-grade; failure => "PRECONDITION FAILED", rows
written, no verdict; a THIRD consecutive PC2 block on this rung escalates
to a consult per the card): PC1 ahat_drift < 0.15 (horizon-scaled, Exp 166
law); PC2 pre-burst stability: expressed preference constant over the 1000
steps before each burst in >= 7/8 forks; PC3 confinement: >= 90% of burst
steps land on burst-color cells.

Predeclared property and falsifier (verbatim from Exp 175):
  P1' (the displacement regime exists): in >= 7/8 forks, for >= 2 of the
     ...
     forks — either the decay resists flips or the baseline recovers on
     its own; no constructible persistent-degradation regime at this
     richness (the Exp 173-class verdict; a real result).
...
Ungated diagnostics: identical to Exp 175.
"""
...
SEEDS = list(range(170, 178))   # 8 fresh forks (175 used 162-169)
...
def run_fork(
    ...
    """
    fork_name = f"exp176_s{fork_seed}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Exp 176 — N4 rung 1 attempt 3, the DISPLACEMENT GATE (settle extension)")
    parser.add_argument("--smoke", action="store_true",
                        help="Smoke run: seed [170] only, full 15000 steps, no verdict")
    args = parser.parse_args()

    seeds = [170] if args.smoke else SEEDS

    print("=" * 80)
    print("Exp 176 — N4 rung 1 attempt 3, the DISPLACEMENT GATE (settle extension)")
    print("=" * 80)
    print()
    ...
    if not pc_pass:
        ...
        out_rows = Path(__file__).parent / "outputs" / "exp176_rows.json"
    ...
    verdict_path = Path(__file__).parent / "outputs" / "exp176_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp176",
        arms=arms,
        ...
        halted=not pc_pass,
        notes=(
            "N4 rung 1 attempt 3, displacement gate with settle extension. LAMBDA=0.9997; "
            "N_STEPS=15000; bursts at 6000/9000/12000 so the inherited near-tie washes out "
            "before the first stability window; displacement = flip + recovery-failure. Forks of mirro only."
        ),
    )
