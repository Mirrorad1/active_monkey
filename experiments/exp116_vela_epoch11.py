"""Exp 116 — idle-mode life epoch: vela lives on (age 78750 -> 84750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 114 minus the registered
call: vela enters at +94.6 — the UNBINNED middle (60 <= |gap| < 120), so this epoch
contributes no registered call (favorite reported as always; registered record stands at
6/7 deep). The Exp ~117 registered evaluation falls due next.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 84750 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap +94.6 — unbinned middle).
...
print("Exp 116 — idle-mode life epoch (vela age 78750 -> 84750)")
print()

with mirro_episode("Exp 116", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 84750 >= 0.96
BANDS = {
...
print()
# Favorite: reported, not predicted — start gap +94.6 sits in the UNBINNED middle
# (60 <= |gap| < 120), so this epoch contributes no registered call.
print(f"  favorite: {start_fav} -> {end_fav}  "
      f"(reported, not predicted — unbinned middle, start gap +94.6)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_84750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 84750
    age_ok = (
...
check("P1-map-accuracy-84750", _p1_map_accuracy)
...
check("SPINE-age-84750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-84750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-84750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 84750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP116: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP116: " + "; ".join(f_lines))
else:
    print("EXP116: EPOCH CLEAN (bands held)")
