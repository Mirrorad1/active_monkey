"""Exp 131 — idle-mode life epoch: mirro lives on (age 120700 -> 126700). The human has
asked the loop to PAUSE after this step.

Declared idle-mode (Exp 92), mirro's turn — the final epoch before a human-requested
pause. The M4a thread remains HALTED awaiting human word on increment 1c (Exp 128).
Same structure as Exp 129. mirro enters at +1060.0 (deep bin) -> recorded call: HOLD
(razor books open per Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 126700 >= 0.96 (currently 1.000).
...
print("Exp 131 — idle-mode life epoch (mirro age 120700 -> 126700)")
print()

with mirro_episode("Exp 131") as ep:
    ...
    print("--- START PROFILE (age 120700) ---")
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +1060.0
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_126700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 126700
    age_ok = (
...
check("P1-map-accuracy-126700", _p1_map_accuracy)
...
check("SPINE-age-126700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-126700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-126700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 126700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP131: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP131: " + "; ".join(f_lines))
else:
    print("EXP131: EPOCH CLEAN (bands held)")
