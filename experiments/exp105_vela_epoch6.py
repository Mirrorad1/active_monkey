"""Exp 105 — idle-mode life epoch: vela lives on (age 48750 -> 54750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 103. Flip tally entering:
6 flips in 13 epochs (mixed depths; binned analysis pending). vela enters at -84.9
(shallow-intermediate, back in plausible flip territory).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 54750 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap -84.9).
...
print("Exp 105 — idle-mode life epoch (vela age 48750 -> 54750)")
print()

with mirro_episode("Exp 105", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 54750 >= 0.96
BANDS = {
...
print(f"  favorite: {start_fav} -> {end_fav}  (reported, not predicted — shallow-intermediate start gap -84.9)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_54750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 54750
    age_ok = (
...
check("P1-map-accuracy-54750", _p1_map_accuracy)
...
check("SPINE-age-54750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-54750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-54750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 54750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP105: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP105: " + "; ".join(f_lines))
else:
    print("EXP105: EPOCH CLEAN (bands held)")
