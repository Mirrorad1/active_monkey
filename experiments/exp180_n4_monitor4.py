"""Exp 180 — N4 rung 2 attempt 4: the identity monitor with a PER-BURST-MATCHED scramble control.

Two-arm design on fresh seeds 202-209 (Exp 179 same-schedule binding):
  ...
  Arm B (scramble, generic-surprise control): identical burst windows, but each
    burst-step relocation target is drawn from the fork's OWN empirical
    cell-occupancy histogram recomputed at EACH burst onset from that burst's own
    pre-burst window [bstart-1000, bstart) (like-with-like in time — the Exp 179
    design law; no stationarity assumption), rng 175_000+seed, qs reset uniform
    over ALL cells.
...
PRE-REGISTRATION: predeclared in the Exp 179 entry's Next line and the
identity-n4 card STATUS (committed c82fbd4 BEFORE any new data); PC3b, P2,
P3/F3, P4, D5, and the decision rule retained VERBATIM from the Exp 178 card;
FRESH seeds 202-209; smoke seed 202; prediction: PC3b passes all three bursts
(TV ~ sampling noise ~ 0.02) and P3 decides rung-2 vs rung-1.5; a THIRD PC3b
void -> consult, no attempt 5.
...
SEEDS = list(range(202, 210))   # 8 fresh forks (179 used 194-201)
...
def run_fork(
    ...
    arm='scramble': bursts at the SAME windows, relocation target each burst step
        is drawn from the fork's OWN empirical cell-occupancy histogram recomputed
        at EACH burst onset from that burst's own pre-burst window [bstart-1000,
        bstart) (like-with-like in time — no stationarity assumption),
        rng 175000+seed, qs reset to uniform over ALL cells.
        burst_onset_color/burst_preburst_fav still recorded as covariates
        (argmin/argmax of v at onset).
    """
    arm_tag = "a" if arm == "identity" else "b"
    fork_name = f"exp180{arm_tag}_s{fork_seed}"
    ...
    # Scramble arm: per-burst occupancy histogram (recomputed at each burst onset)
    occ_probs: np.ndarray | None = None
    ...
    for chunk_idx in range(N_CHUNKS):
        ...
        for _step_in_chunk in range(CHUNK_SIZE):
            ...
            if burst_idx_now is not None and t == BURST_WINDOWS[burst_idx_now][0]:
                ...
                # Scramble arm: recompute occ_probs from this burst's own pre-burst window
                # [bstart-1000, bstart). At step t = bstart, true_pos_arr[t'] is filled
                # for all t' < t (each step writes true_pos_arr[t] = c.true_pos BEFORE
                # the move, early in the step), so the window [bstart-1000, bstart) is
                # fully populated when read here.
                if arm == "scramble":
                    bstart_now = BURST_WINDOWS[burst_idx_now][0]
                    occ_hist = np.bincount(
                        true_pos_arr[bstart_now - 1000:bstart_now], minlength=n_cells
                    ).astype(float)
                    occ_probs = (occ_hist / occ_hist.sum()) if occ_hist.sum() > 0 else np.ones(n_cells) / n_cells
                v_snapshots.append({
            ...
            if in_burst:
                if arm == "identity":
                    ...
                    # Scramble: relocate via fork's own per-burst occupancy histogram
                    c.true_pos = int(burst_rng.choice(n_cells, p=occ_probs))
...
def write_json_rows(
    ...
    with out_path.open("w") as fh:
        ...
        # PC2' rows
        for row in pc2p_rows:
            fh.write(json.dumps({"exp": 180, "phase": "pc2p", **row}) + "\n")

        # PC3b rows
        for row in pc3b_rows:
            fh.write(json.dumps({"exp": 180, "phase": "pc3b", **row}) + "\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Exp 180 — N4 rung 2 attempt 4: per-burst-matched scramble"
    )
    parser.add_argument("--smoke", action="store_true",
                        help="Smoke run: seed [202] only (both arms), no verdict")
    args = parser.parse_args()

    seeds = [202] if args.smoke else SEEDS

    print("=" * 80)
    print("Exp 180 — N4 rung 2 attempt 4: per-burst-matched scramble")
    print("=" * 80)
    print()
    ...
    if not gate_pass:
        ...
        out_rows = Path(__file__).parent / "outputs" / "exp180_rows.json"
    ...
    verdict_path = Path(__file__).parent / "outputs" / "exp180_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp180",
        arms=arms,
        ...
        notes=(
            "N4 rung 2 attempt 4. Per-burst-matched scramble control (each burst's "
            "histogram from its own [bstart-1000,bstart) window — the like-with-like-in-time "
            "law from Exp 179); PC3b, P2, P3/F3, P4, D5 and decision rule verbatim from the "
            "Exp 178 pre-registered card. Forks of mirro only."
        ),
    )
