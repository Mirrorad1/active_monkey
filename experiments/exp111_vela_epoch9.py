"""Exp 111 — idle-mode life epoch: vela lives on (age 66750 -> 72750). Registered deep
call #4.

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 109. Registered record
entering: 3/3 deep. vela enters at -261.8 (deep bin) -> registered call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 72750 >= 0.96 (currently 1.000).
...
print("Exp 111 — idle-mode life epoch (vela age 66750 -> 72750)")
print()

with mirro_episode("Exp 111", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 72750 >= 0.96
BANDS = {
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at -261.8
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_72750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 72750
    age_ok = (
...
check("P1-map-accuracy-72750", _p1_map_accuracy)
...
check("SPINE-age-72750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-72750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-72750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 72750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP111: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP111: " + "; ".join(f_lines))
else:
    print("EXP111: EPOCH CLEAN (bands held)")
