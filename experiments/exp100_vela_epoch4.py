"""Exp 100 — idle-mode life epoch: vela lives on (age 36750 -> 42750). The centennial
entry is, fittingly, a creature simply living.

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 98. Flip tally entering:
5 flips in 9 shallow-gap epochs (the model's ~40 percent on the nose). vela enters at an
intermediate depth (-60.4) — another datum for the depth record either way.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 42750 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap -60.4 — intermediate).
...
print("Exp 100 — idle-mode life epoch (vela age 36750 -> 42750)")
print()

with mirro_episode("Exp 100", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 42750 >= 0.96
BANDS = {
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_42750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 42750
    age_ok = (
...
check("P1-map-accuracy-42750", _p1_map_accuracy)
...
check("SPINE-age-42750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-42750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-42750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 42750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP100: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP100: " + "; ".join(f_lines))
else:
    print("EXP100: EPOCH CLEAN (bands held)")
