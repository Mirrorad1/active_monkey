"""Exp 104 — idle-mode life epoch: mirro lives on (age 54700 -> 60700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 101. Flip tally
entering: 6 flips in 12 epochs (denominator mixes depths; binned analysis pending).
mirro enters at intermediate depth (-128.2).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 60700 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap -128.2 — intermediate).
...
print("Exp 104 — idle-mode life epoch (mirro age 54700 -> 60700)")
print()

with mirro_episode("Exp 104") as ep:
...
print(f"  favorite: {start_fav} -> {end_fav}  (reported, not predicted — intermediate-depth start gap -128.2)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_60700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 60700
    age_ok = (
...
check("P1-map-accuracy-60700", _p1_map_accuracy)
...
check("SPINE-age-60700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-60700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-60700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 60700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP104: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP104: " + "; ".join(f_lines))
else:
    print("EXP104: EPOCH CLEAN (bands held)")
