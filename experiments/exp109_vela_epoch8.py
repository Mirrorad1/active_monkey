"""Exp 109 — idle-mode life epoch: vela lives on (age 60750 -> 66750). Registered deep
call #2.

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 107. The registered
depth-law test (Exp 107) is live; vela enters at -125.8 (deep bin, |gap| >= 120) ->
registered call: HOLD. Registered record entering: 1/1 deep.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 66750 >= 0.96 (currently 1.000).
...
print("Exp 109 — idle-mode life epoch (vela age 60750 -> 66750)")
print()

with mirro_episode("Exp 109", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 66750 >= 0.96
BANDS = {
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at -125.8
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_66750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 66750
    age_ok = (
...
check("P1-map-accuracy-66750", _p1_map_accuracy)
...
check("SPINE-age-66750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-66750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-66750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 66750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP109: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP109: " + "; ".join(f_lines))
else:
    print("EXP109: EPOCH CLEAN (bands held)")
