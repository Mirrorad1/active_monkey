"""Exp 97 — idle-mode life epoch: mirro lives on (age 36700 -> 42700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 95. Flip-record tally
entering this epoch: 5 flips in 6 shallow-gap epochs (Exp 76, 81, 93, 94, 96 flipped;
Exp 95 held) vs the noise model's ~40 percent — small-n, running hot, tracked.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 42700 >= 0.96 (currently 1.000).
  ...
  FAVORITE: reported, not predicted (start gap +40.3 — shallow).
...
print("Exp 97 — idle-mode life epoch (mirro age 36700 -> 42700)")
print()

with mirro_episode("Exp 97") as ep:
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_42700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 42700
    age_ok = (
...
check("P1-map-accuracy-42700", _p1_map_accuracy)
...
check("SPINE-age-42700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-42700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-42700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 42700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP97: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP97: " + "; ".join(f_lines))
else:
    print("EXP97: EPOCH CLEAN (bands held)")
