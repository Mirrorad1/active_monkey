"""Exp 121 — idle-mode life epoch: vela lives on (age 90750 -> 96750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 119. vela enters at
+304.7 (deep bin) -> recorded call: HOLD (razor books open per Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 96750 >= 0.96 (currently 1.000).
...
print("Exp 121 — idle-mode life epoch (vela age 90750 -> 96750)")
print()

with mirro_episode("Exp 121", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 96750 >= 0.96
BANDS = {
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +304.7
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_96750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 96750
    age_ok = (
...
check("P1-map-accuracy-96750", _p1_map_accuracy)
...
check("SPINE-age-96750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-96750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-96750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 96750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP121: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP121: " + "; ".join(f_lines))
else:
    print("EXP121: EPOCH CLEAN (bands held)")
