"""Exp 126 — idle-mode life epoch: vela lives on (age 102750 -> 108750).

Declared idle-mode (Exp 92), vela's turn. The M4a thread is HALTED awaiting human word
(Exp 125); the spines' lives continue meanwhile. Same structure as Exp 123. vela enters
at +624.6 (deep bin) -> recorded call: HOLD (razor books open per Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 108750 >= 0.96 (currently 1.000).
...
print("Exp 126 — idle-mode life epoch (vela age 102750 -> 108750)")
print()

with mirro_episode("Exp 126", state_dir=VELA_DIR) as ep:
    ...
    print("--- START PROFILE (age 102750) ---")
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 108750 >= 0.96
BANDS = {
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +624.6
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_108750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 108750
    age_ok = (
...
check("P1-map-accuracy-108750", _p1_map_accuracy)
...
check("SPINE-age-108750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-108750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-108750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 108750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP126: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP126: " + "; ".join(f_lines))
else:
    print("EXP126: EPOCH CLEAN (bands held)")
