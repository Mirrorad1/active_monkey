"""Exp 174 — N4 rung 1, the WHIPSAW GATE: a constructible transient-pressure
regime must flip the N4-less baseline's expressed preference and let it
recover — the anti-regress precondition for the identity layer (card:
loop/directions/identity-n4.md).

Hypothesis: under window-decayed values (lambda = 0.999 per step — the
Exp 88 window-theorem class, the program's sanctioned lifelong-adaptation
configuration; DECLARED, not tuned: raw accumulated counts at mirro's mass
provably cannot flip transiently), a forced-relocation burst (the fork is
relocated each step to a uniformly random cell of the burst color for 800
steps — world-driven captivity) makes the burst color's decayed value mass
overtake the standing favorite (FLIP), and free roaming thereafter restores
the prior ordering (RECOVERY).

Design: forks of mirro only (spine never saved), FRESH seeds 154-161, one
12000-step session per fork. Value system: v[c] decays by lambda each step,
then v[obs] += predictability_weight (the Exp 26 rule on the decayed store;
v initialized from the spine's normalized value_counts scaled to mass
1/(1-lambda) = 1000 so the standing preference is inherited at the decay
equilibrium scale — declared). Expressed preference at t = argmax v.
Bursts at steps 3000-3799, 6000-6799, 9000-9799; burst color = the argmin
of v at burst onset (per fork, per burst). During a burst the move step is
replaced by relocation to a uniformly random cell of the burst color (own
rng 160_000 + fork_seed); outside bursts the standard random walk runs.
...
1000 steps before each burst in >= 7/8 forks (the thing being flipped must
exist); PC3 confinement: >= 90% of burst steps land on burst-color cells.

Predeclared property and falsifier:
  P1 (the whipsaw exists): in >= 7/8 forks, for >= 2 of the 3 bursts:
     (i) FLIP — the expressed preference equals the burst color for >= 50%
     of the burst's final 400 steps; and (ii) RECOVERY — the pre-burst
     favorite is restored as expressed preference within 2000 steps after
     burst end and holds for >= 500 consecutive steps.
     FALSIFIER F1: flip-with-recovery achieved in <= 4/8 forks — no
     constructible whipsaw at this richness; the gate FAILS and the N4
     ladder is untestable here (the Exp 173-class verdict; a real result).
  VERDICT: POSITIVE (GATE PASSES — the discriminating transient-pressure
  regime exists; rungs 2-3 are armed) iff P1. NEGATIVE iff F1. Otherwise
  MIXED. "Not a falsifier" never counts toward POSITIVE.

Ungated diagnostics: per-fork value trajectories at burst boundaries
(v of favorite and burst color at onset / burst end / +1000 / +2000);
flip latency and recovery latency per burst; the inherited standing
favorite per fork; drift; confinement rates.
"""
...
SEEDS = list(range(154, 162))   # 8 fresh forks
...
# Value decay
LAMBDA = 0.999          # per-step decay; equilibrium mass = 1/(1-lambda) = 1000
INIT_MASS = 1.0 / (1.0 - LAMBDA)  # = 1000.0
...
def run_fork(
    ...
    """
    fork_name = f"exp174_s{fork_seed}"
...
def evaluate(run_results: list[dict]) -> dict:
    ...
    for rr in run_results:
        ...
        bursts_with_flip_recovery = 0

        for bi in range(3):
            ba = analyze_burst(expressed_arr, bi, burst_preburst_fav, burst_onset_color)
            burst_results.append(ba)
            if ba["flip"] and ba["recovery"]:
                bursts_with_flip_recovery += 1

        fork_passes = bursts_with_flip_recovery >= P1_MIN_BURSTS_PER_FORK
        if fork_passes:
            forks_with_whipsaw += 1

        per_fork_details.append({
            "fork_seed": seed,
            "burst_results": burst_results,
            "bursts_with_flip_recovery": bursts_with_flip_recovery,
            "fork_passes": fork_passes,
            "standing_fav": rr["burst_preburst_fav"][0],
        })

    p1_pass = forks_with_whipsaw >= P1_MIN_FORKS
    f1 = forks_with_whipsaw <= F1_MAX_FORKS
    ...
    return {
        "forks_with_whipsaw": forks_with_whipsaw,
        "p1_pass": p1_pass,
        "f1": f1,
    ...
    with out_path.open("w") as fh:
        ...
        if ev is not None:
            summary = {
                "exp": 174,
                "phase": "summary",
                "forks_with_whipsaw": ev["forks_with_whipsaw"],
                "p1_pass": ev["p1_pass"],
                "f1": ev["f1"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Exp 174 — N4 whipsaw gate")
    parser.add_argument("--smoke", action="store_true",
                        help="Smoke run: seed [154] only, full 12000 steps, no verdict")
    args = parser.parse_args()

    seeds = [154] if args.smoke else SEEDS

    print("=" * 80)
    print("Exp 174 — N4 rung 1, the WHIPSAW GATE")
    print("=" * 80)
    print()
    ...
    if not pc_pass:
        ...
        out_rows = Path(__file__).parent / "outputs" / "exp174_rows.json"
    ...
    for fd in ev["per_fork_details"]:
        bsums = [
            f"b{bi}:{'FLIP+REC' if br['flip'] and br['recovery'] else ('FLIP' if br['flip'] else 'none')}"
            for bi, br in enumerate(fd["burst_results"])
        ]
        status = "PASS" if fd["fork_passes"] else "fail"
        print(f"  seed={fd['fork_seed']} bursts_ok={fd['bursts_with_flip_recovery']}/3 "
              f"[{', '.join(bsums)}] => {status}")

    print()
    p1_status = "PASS" if ev["p1_pass"] else ("FALSIFIER F1" if ev["f1"] else "MIXED")
    print(f"P1 (whipsaw exists): forks_with_whipsaw={ev['forks_with_whipsaw']}/8 "
    ...
    print("=" * 80)
    print()
    print(f"P1 (whipsaw exists, >= {P1_MIN_FORKS}/8 forks, >= {P1_MIN_BURSTS_PER_FORK}/3 bursts): "
          f"{p1_status}")
    print()
    if ev["verdict_str"] == "POSITIVE":
        gate_text = "GATE PASSES — discriminating transient-pressure regime exists; rungs 2-3 are armed"
    elif ev["verdict_str"] == "NEGATIVE":
        gate_text = ("GATE FAILS — no constructible whipsaw at this richness; "
                     "N4 ladder untestable here (Exp 173-class result)")
    else:
    ...
    arms = {
        "P1_whipsaw_exists": {
            ...
            "reason": (
                f"forks_with_whipsaw={ev['forks_with_whipsaw']}/8 "
                f"(need >={P1_MIN_FORKS}; F1: <={F1_MAX_FORKS}); "
                f"flip+recovery in >= {P1_MIN_BURSTS_PER_FORK}/3 bursts per qualifying fork"
            ),
        },
    }
    verdict_path = Path(__file__).parent / "outputs" / "exp174_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp174",
        arms=arms,
        ...
        halted=not pc_pass,
        notes=(
            "N4 rung 1 whipsaw gate. LAMBDA=0.999 decayed value store; "
