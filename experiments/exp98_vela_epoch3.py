"""Exp 98 — idle-mode life epoch: vela lives on (age 30750 -> 36750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 96. Flip tally entering:
5 flips in 7 shallow-gap epochs. The Exp 96 watch-item is live: vela enters at its
deepest-ever gap (-101.5) — if the margin has stopped being shallow, the oscillation
should slow; the favorite remains REPORTED, not predicted, but the gap depth makes a
hold likelier than at previous razor margins (stated, not banded).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 36750 >= 0.96 (currently 1.000).
...
print("Exp 98 — idle-mode life epoch (vela age 30750 -> 36750)")
print()

with mirro_episode("Exp 98", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 36750 >= 0.96
BANDS = {
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_36750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 36750
    age_ok = (
...
check("P1-map-accuracy-36750", _p1_map_accuracy)
...
check("SPINE-age-36750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-36750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-36750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 36750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP98: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP98: " + "; ".join(f_lines))
else:
    print("EXP98: EPOCH CLEAN (bands held)")
