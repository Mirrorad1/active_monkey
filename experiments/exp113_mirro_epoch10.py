"""Exp 113 — idle-mode life epoch: mirro lives on (age 78700 -> 84700). Registered deep
call #5.

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 110. Registered record
entering: 3/4 deep (the Exp 111 miss spent the tolerance; Exp 112 measured the deep flip
rate at 0.20 and the registration stands as written). mirro enters at +497.5 (deep bin)
-> registered call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 84700 >= 0.96 (currently 1.000).
  P2 (value core): |delta share_c| < 0.04 each and |delta conviction| < 0.04.
  P3 (noise-calibrated forecast): end gap c2-c0 within predicted +- 302.5 (2.5 sigma at
     the REGISTERED shallow-state sigma; per Exp 112, deep-state tails are read against
     that entry).
...
print("Exp 113 — idle-mode life epoch (mirro age 78700 -> 84700)")
print()

with mirro_episode("Exp 113") as ep:
...
print()
# Favorite: registered depth call (P4) — predicted HOLD, deep bin entry at +497.5
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_84700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 84700
    age_ok = (
...
check("P1-map-accuracy-84700", _p1_map_accuracy)
...
check("SPINE-age-84700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-84700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-84700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 84700 below 0.96; knowledge "
...
if f3_fired:
    ...
    print(f"  F3 FIRED: P3 fails — gap outside 2.5-sigma band "
          ...
          "a >2.5-sigma event — interpreted against Exp 112's state-dependent "
          "sigma finding.")
...
if spine_failed:
    print("EXP113: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP113: " + "; ".join(f_lines))
else:
    print("EXP113: EPOCH CLEAN (bands held)")
