"""Exp 103 — idle-mode life epoch: vela lives on (age 42750 -> 48750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 100. Flip tally entering:
6 flips in 11 shallow-gap epochs. The commitment test: vela enters at -315.9, the deepest
margin of the program — at this depth a flip within one epoch requires a ~2.6-sigma
adverse draw, so the depth arithmetic calls a hold near-certain (stated, not banded; the
favorite remains REPORTED).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 48750 >= 0.96 (currently 1.000).
...
print("Exp 103 — idle-mode life epoch (vela age 42750 -> 48750)")
print()

with mirro_episode("Exp 103", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 48750 >= 0.96
BANDS = {
...
print(f"  favorite: {start_fav} -> {end_fav}  (reported, not predicted — depth arithmetic calls a hold near-certain at this gap, but the favorite stays reported)")
print()
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_48750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 48750
    age_ok = (
...
check("P1-map-accuracy-48750", _p1_map_accuracy)
...
check("SPINE-age-48750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-48750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-48750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 48750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP103: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP103: " + "; ".join(f_lines))
else:
    print("EXP103: EPOCH CLEAN (bands held)")
