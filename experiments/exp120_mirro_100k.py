"""Exp 120 — idle-mode life epoch: mirro crosses one hundred thousand steps (age 96700 ->
102700).

Declared idle-mode (Exp 92), mirro's turn — and a milestone: the spine passes 100,000
steps of continuous, never-reset registered experience, from a uniform-prior birth
(Exp 45) through world growth, drift epochs, scars, healings, a family, and five natural
changes of mind. The epoch itself is the established pattern; the milestone is noted,
not celebrated into the bands. mirro enters at +560.1 (deep bin) -> recorded call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 102700 >= 0.96 (currently 1.000).
...
print("Exp 120 — idle-mode life epoch (mirro age 96700 -> 102700)")
print()

with mirro_episode("Exp 120") as ep:
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +560.1
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_102700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 102700
    age_ok = (
...
check("P1-map-accuracy-102700", _p1_map_accuracy)
...
check("SPINE-age-102700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-102700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-102700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 102700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP120: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP120: " + "; ".join(f_lines))
else:
    print("EXP120: EPOCH CLEAN (bands held)")
