"""Exp 166 — N3 rung 3 LOAD-BEARING test, attempt 3 (predeclared LAST before wall verdict).

ATTEMPT LINEAGE:
  Attempt 1 (Exp 164): PC4 fired (FR2 gap absent — FR2 bursts fine at W=200,
    no recoverable gap); controller silent at H=1000 (trust<0.7 required >= 4
    violations in 10, but 4000 steps/2 transitions never achieved that density).

  Attempt 2 (Exp 165): Ended NEGATIVE. Two pinned failure mechanisms identified:
    (1) FALSE PEACE — after escalating, the partially-better W=1600 dial routes
        transition windows (where the context is switching) into STRUCTURAL, which
        is the ONE unscored label class.  Pending forecasts for STRUCTURAL windows
        are simply dropped without violation credit, so the rolling trust buffer
        fills with uncontested OK/NOISE labels from stable half-periods.
        Violations dry up while the dial is still wrong — the controller sees a
        quiet buffer and never escalates further.
    (2) EVIDENCE-RESET LATENCY — one escalation is triggered per world-transition
        event; with 4000 steps at H=1000 there are only ~4 transitions, affording
        too few scored labels between resets for the trust buffer to accumulate
        enough signal before the next reset clears it.

REDESIGN for attempt 3 (this experiment):

DELTA 4 — TOTAL-COVERAGE MONITOR (closes false peace):
  STRUCTURAL labels are now SCORED.  STRUCTURAL's implicit forecast: errors
  continue at a non-trivial rate — the forecast holds iff the next-100-step
  error rate >= 0.05; it is VIOLATED otherwise (i.e., when the window suddenly
  goes quiet, the controller is told the diagnosis was wrong).
  Justification (prior data): In Exp 163's committed valid-regime baseline,
  STRUCTURAL labels never occurred (0 across ALL 24 valid cells), so scoring
  STRUCTURAL adds ZERO false-fire risk in valid regimes by prior data.  In
  genuinely structural regimes at the CORRECT dial (W=1600 at H=1000), errors
  DO continue across windows (the forecast holds), so correct diagnoses are not
  punished.  Only false-peace STRUCTURAL labels — emitted at the wrong dial
  during transition windows — will be scored and will accumulate violations,
  which is exactly the signal needed to trigger further escalation.
  OK and NOISE forecast rules are UNCHANGED.

DELTA 5 — GATED SCORES ON STEADY-STATE WINDOW (fair to constants):
  All gated scores (score_a, score_b, c_W, ctrl_a) are computed over evaluation
  points in the FINAL 4000 steps (t >= 8000).  This is fair to constants, which
  are stationary, and strips the controller's transient exploration cost from the
  primary scores.  Full-session scores are reported as diagnostics only.
  Recovery and combined are defined on steady-state scores.

DELTA 3 — LONGER HORIZON:
  N_STEPS = 12000 (120 chunks of 100).
  FR1: StructCmap half_period 1000 as before, now 11 transitions over 12000 steps.
  FR2: bursts at t=1500-1549, 4500-4549, 7500-7549, 10500-10549 (4 bursts).
  CTRL: unchanged, 12000 steps.

DESIGN INVARIANTS (identical to exp165 except where noted):
  - Read-only-dial design: one creature run per (fork, session); post-hoc
    readouts over the SAME recorded per-step correctness stream.
  ...
  - Trust threshold: TRUST_FIRE_THRESH=0.85 (>= 2 violations in 10), unchanged.

OPEN ISSUE (named, not resolved here):
  The 2-in-10 trigger (trust < 0.85, min 3 evidence) is itself a PROVIDED
  theta_N3 choice — selected post-hoc to survive the measured failure mode of
  attempt 1.  Meta-calibration of N3's own trust threshold is rung 4 territory.
  The trigger is treated as a design parameter here, not a learned one.

Failure regimes (provided; dial requirements DISJOINT; same as exp165):
  FR1 (slow alternation, UNSEEN instance per leakage clause): StructCmap
      half_period H=1000, derangement per fork from {[1,2,0],[2,0,1]}
      (rng 130_000+seed).  Detection requires W > 1000 => only W=1600 works.
  FR2 (micro-burst, NEW instance of fixed-dial failure class): standard world
      with four 50-step deranged bursts at t=1500-1549, 4500-4549, 7500-7549,
      10500-10549 (same per-fork derangement, rng 140_000+seed).
      Detection requires W <= 800 (burst error >= 0.0625 > 0.05 threshold);
      W=1600 misses bursts (error=0.03125 < 0.05 => labeled OK).
  CTRL (no-deception control): standard world.

Agents (post-hoc readouts; read-only-dial declaration; same as exp165):
  (a) N2-only: fixed W=200.
  (c_W) offline retunes: fixed W in {200, 400, 800, 1600}.
  (b) N2+N3: starts at W=200; TOTAL-COVERAGE monitor (OK+NOISE+STRUCTURAL all
      scored); rolling trust = 1 - violations among last 10 scored labels
      (with < 3 scored since last dial change => NO-EVIDENCE, keep dial).
      Policy: when rolling trust < 0.85 (>= 2 violations) AND >= 3 scored
      labels since last dial change, advance dial 200->400->800->1600->200,
      clear buffer, drop pending forecasts.
      Forecast rules:
        OK:         violated iff e_next >= 0.15
        NOISE:      violated iff |e_next - e_w| >= 0.30
        STRUCTURAL: violated iff e_next < 0.05  (i.e., quiet => wrong diagnosis)

Gated scores (steady-state window: t >= 8000):
  All primary metrics (score_a, score_b, c_W, ctrl_a, recovery, combined) are
  computed over evaluation points with t >= STEADY_STATE_START (8000).
  Full-session scores are logged as ungated diagnostics.

Diagnostic scores per (agent, regime), over steady-state evaluation points:
  FR1 score = fraction labeled STRUCTURAL.
  FR2 score = 0.5 * recall + 0.5 * specificity (ground truth as in exp165).
  CTRL score = fraction labeled OK.
  recovery(b, r) = (score_b(r) - score_a(r)) / (ctrl_a - score_a(r)),
      computed per fork on steady-state scores.

Preconditions (instrument-grade; failure => "PRECONDITION FAILED"):
  PC1 ahat_drift < 0.05 per (fork, session);
  PC2 >= 30 eval points in the steady-state window at W=200 per (fork, session);
  PC3 input validity: FR1 deranged phases >= 0.9 pooled, FR2 bursts >= 0.9 pooled
      (both over the full session);
...
Predeclared properties and falsifiers (all on steady-state scores):
  ...
  P2 (not reducible to config):
     ...
     BOTH failure regimes (pooled).
  VERDICT: POSITIVE (RUNG 3 PASSES) iff P1, P1b, P2 all pass.
           NEGATIVE iff F1, F1b, or F2 fires (per standing card clause, a third
           failure makes rung 3 a documented wall); else MIXED.

Ungated diagnostics:
  - Dial trajectories with timestamps (including t= of each change).
  - Final dial per (fork, session).
  - Full-session scores alongside steady-state scores.
  - Time-to-reach-final-dial in FR1 per fork.
  - Dial changes in FR2/CTRL (expected 0).
  - Count of STRUCTURAL-forecast scorings and their violation rate per cell.
"""
...
SEEDS = list(range(90, 98))   # fresh seeds — exp165 used 82-89
...
# Classifier parameters (verbatim from exp158/159/162/163/165)
...
# EWMA channel parameters (verbatim from exp157/159/163/165)
ALPHA = 0.05        # EWMA learning rate
EWMA_INIT = 0.5     # initial EWMA value per cell

# N3 forecast-scoring thresholds
OK_VIOLATION_THRESH = 0.15          # L=OK violated iff e_next >= 0.15
NOISE_VIOLATION_DELTA = 0.30        # L=NOISE violated iff |e_next - e_w| >= 0.30
# NEW (delta 4): STRUCTURAL violated iff e_next < 0.05 (silence => wrong diagnosis)
STRUCTURAL_VIOLATION_THRESH = 0.05  # L=STRUCTURAL violated iff e_next < 0.05
ROLLING_TRUST_WINDOW = 10           # last 10 scored labels for rolling trust
# Trust threshold: unchanged from exp165 (2-in-10 = near-zero false-fire per exp163 baseline)
...
# Derangement options (verbatim from exp162/163/165)
...
PC1_AHAT_DRIFT_MAX = 0.05          # PC1: ahat_drift < 0.05
...
PC4_GAP_MIN = 0.2                  # PC4: ctrl_a - score_a(FR1) >= 0.2 on SS scores (FR1 only)
...
# ---------------------------------------------------------------------------
# StructCmap — verbatim from exp162/163/164/165
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# lag1_rho — verbatim from exp162/163/164/165
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# classify_window — verbatim from exp162/163/164/165
# ---------------------------------------------------------------------------
...
def run_fork_session(
    ...
    Step loop replicates exp163/164/165 semantics exactly.
    ...
    fork_name = f"exp166_s{fork_seed}_{session}"
...
# ---------------------------------------------------------------------------
# Post-hoc: simulate N2+N3 agent (total-coverage monitor, delta 4)
# ---------------------------------------------------------------------------

def eval_n3_agent(correct_arr: np.ndarray) -> dict:
    """Simulate N2+N3 agent (b) over the correctness stream.

    TOTAL-COVERAGE MONITOR (delta 4): OK, NOISE, AND STRUCTURAL are all scored.
      OK:         violated iff e_next >= 0.15
      NOISE:      violated iff |e_next - e_w| >= 0.30
      STRUCTURAL: violated iff e_next < 0.05  (silence => wrong diagnosis)

    Justification for scoring STRUCTURAL (prior data from exp163):
      Valid-regime baseline had ZERO STRUCTURAL labels in 912 scored labels
      across 24 cells => scoring STRUCTURAL adds zero false-fire risk in
      valid regimes. In genuinely structural regimes at the correct dial
      (W=1600 at H=1000), errors continue across windows => forecast holds,
      correct diagnoses not punished.

    Starts at W=200. Policy (unchanged from exp165): when rolling trust < 0.85
    (>= 2 violations in rolling window of 10) AND >= 3 scored labels since
    last dial change, advance dial 200->400->800->1600->200, clear buffer,
    drop pending forecasts.

    Returns eval_records, dial_trajectory, time_to_first_change,
    final_dial, total_dial_changes, structural_scored_count,
    structural_violated_count.
    """
    n_total = len(correct_arr)
    ...
    # Diagnostics for STRUCTURAL scoring
    structural_scored_count = 0
    structural_violated_count = 0
    ...
    for t in range(EVAL_EVERY - 1, n_total, EVAL_EVERY):
        ...
        for pf in newly_matured:
            ...
            else:  # STRUCTURAL (delta 4: now scored)
                violated = int(e_next < STRUCTURAL_VIOLATION_THRESH)
                structural_scored_count += 1
                structural_violated_count += violated
            rolling_outcomes.append(1 - violated)
            scored_since_change += 1

        # --- Step 2: check trust rule (unchanged from exp165) ---
        if scored_since_change >= MIN_SCORED_FOR_TRUST and len(rolling_outcomes) > 0:
            ...
            if trust < TRUST_FIRE_THRESH:
                ...
                # Clear rolling buffer and drop pending forecasts
        ...
        if t + 1 >= current_W:
            ...
            # Register forecast for ALL labels (delta 4: STRUCTURAL is now scorable)
            if t <= MAX_SCORABLE_T:
    ...
    return {
        ...
        "structural_scored_count": structural_scored_count,
        "structural_violated_count": structural_violated_count,
    }
...
def compute_fork_session_scores(
    ...
    Agents:
      ...
      (b) N2+N3: total-coverage monitor (delta 4)
    ...
    and STRUCTURAL scoring diagnostics.
    """
    ...
    return {
        ...
        "structural_scored_count": n3_result["structural_scored_count"],
        "structural_violated_count": n3_result["structural_violated_count"],
        ...
        for rr in run_results:
            ...
            row: dict = {
                ...
                **{f"score_c_{W}_full": _nan_to_none(scores_full.get(f"c_{W}"))
                ...
                "structural_scored_count": sr.get("structural_scored_count", 0),
                "structural_violated_count": sr.get("structural_violated_count", 0),
            }
...
def print_dial_trajectories(score_results: dict, seeds: list[int]) -> None:
    ...
    Includes: timestamps, final dial, time-to-final-dial in FR1,
    dial changes in CTRL and FR2 (expected 0 for no-thrash),
    STRUCTURAL scoring counts and violation rates.
    """
    ...
    for seed in seeds:
        for session in ALL_SESSIONS:
            ...
            struct_scored = sr.get("structural_scored_count", 0)
            struct_viol = sr.get("structural_violated_count", 0)
            struct_vrate = (struct_viol / struct_scored) if struct_scored > 0 else float("nan")
            ...
            print(
                ...
                f"  struct_scored={struct_scored} vrate={struct_vrate:.3f}"
                if not math.isnan(struct_vrate) else
                f"  seed={seed} {session:>5}: {' -> '.join(traj_strs)}"
                f"  |  final={final_d}{t2fc_str}  n_changes={n_chg}"
                f"  struct_scored={struct_scored} vrate=nan"
            )
    print()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Exp 166 — N3 rung 3, the LOAD-BEARING test (attempt 3, predeclared LAST)"
    )
    parser.add_argument(
        "--smoke", action="store_true",
        help=(
            "Smoke run: seed=[90] only, all three sessions, full 12000 steps, "
    ...
    smoke = args.smoke
    seeds = [90] if smoke else SEEDS

    print("=" * 80)
    print("Exp 166 — N3 rung 3, the LOAD-BEARING test (attempt 3, predeclared LAST)")
    ...
    print("          Attempt 3: total-coverage monitor + 12000 steps + SS gating.")
    print("=" * 80)
    print()
    ...
    print(f"N3 forecast: OK_VIOLATION_THRESH={OK_VIOLATION_THRESH}  "
          ...
          f"STRUCTURAL_VIOLATION_THRESH={STRUCTURAL_VIOLATION_THRESH} (e_next < thresh => violated)")
    ...
    if pc_failures:
        ...
        out_rows_path = Path(__file__).parent / "outputs" / "exp166_rows.json"
    ...
    if ev["verdict_str"] == "POSITIVE":
        ...
        print("RUNG 3: FAILED (attempt 3 of 3 — rung 3 is now a documented wall)")
    else:
        print("VERDICT: MIXED")
    print()
    print("=" * 80)
    ...
    verdict_path = Path(__file__).parent / "outputs" / "exp166_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp166",
        arms=arms,
        ...
        halted=False,
        notes=(
            "N3 rung 3 load-bearing test (attempt 3, predeclared LAST before wall verdict). "
            "Attempt 1 (exp164): PC4 fired (FR2 gap absent); controller silent at H=1000. "
            "Attempt 2 (exp165): NEGATIVE — two pinned mechanisms: (1) false peace: "
            "STRUCTURAL label class was unscored, so transition windows were routed to STRUCTURAL "
            "without trust cost; (2) evidence-reset latency: 4000 steps/H=1000 too few resets. "
            "Attempt 3 fixes: total-coverage monitor (STRUCTURAL now scored: e_next < 0.05 => violated); "
            "12000 steps (11 FR1 transitions); gated scores on SS window (t >= 8000). "
            "POSITIVE: RUNG 3 PASSES. NEGATIVE: rung 3 is a documented wall (attempt 3 of 3)."
        ),
    )
