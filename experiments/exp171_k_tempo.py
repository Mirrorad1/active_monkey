"""Exp 171 — N3 K-endogenization tier B: recency-weighted, outlier-robust horizon.
...
RATCHET LINEAGE:
  ...
  Exp 170 (tier A): global P95 of completed run lengths.  FAILED — the pooled
  percentile is outlier-dominated by long STRUCTURAL plateaus, collapsing to
  premature-K=3-then-permanent-K=16 (the tier-A collapse).

THIS EXPERIMENT — K-ENDOGENIZATION TIER B (everything else verbatim from exp170):

  TIER-B DERIVED K (replaces exp170's global-P95 rule):
    K(t) = clamp(ceil(2 * median(last M=12 completed run lengths)) + 1, 3, 16)
    with no completed runs: K(t) = 3.
    Run completion/discard rules UNCHANGED from exp170 (complete on class change;
    discard on dial change).
    Rationale: the MEDIAN is outlier-robust — one long plateau completing at a
    regime boundary cannot ratchet the horizon (tier A's collapse); the SLIDING
    WINDOW of the last M=12 completions makes the horizon follow the stream's
    CURRENT cycling tempo (fast flicker -> small K, locks form quickly when
    settled; slow cycling -> larger K, the lock outwaits the cycle); the
    multiplier 2 places the bar just beyond the typical demonstrated run; clamp
    [3,16], M=12, and the multiplier are the remaining provided policy, declared.

  DELTAS FROM EXP 170:
    - SEEDS = list(range(130, 138)) (exp170 used 122-129)
    - Output IDs: exp171 (rows -> experiments/outputs/exp171_rows.json,
      verdict -> exp171_verdict.json, experiment="exp171")
    - THREE same-schedule arms, all read-only readouts of the SAME recorded
      per-step stream:
        (a)  fixed W=200 baseline
        (b8) Exp 169 controller verbatim (constant K=8, descent driver, no wrap)
        (bB) Tier-B controller (identical to b8 except K(t) as above)
      All P/F comparisons of bB use b8's SAME-RUN pooled values, eliminating
      the schedule confound.
    - Tier-B K formula replaces P95; M=12 sliding window; multiplier=2.

  EVERYTHING ELSE VERBATIM FROM EXP 170 / EXP 169:
    Mixed schedule, StructCmap/NoisyCmap/RandomPlaceNoisyCmap, scoring,
...
SEEDS = list(range(130, 138)) — 8 fresh forks; exp170 used 122-129.

MIXED SESSION — one per fork (verbatim from exp168/169/170):
...
AGENTS (post-hoc readouts; read-only-dial):
  ...
  (bB) N2+N3 COMPLETED controller with TIER-B derived K(t).

SCORING per segment (verbatim from exp168/169/170):
...
PRECONDITIONS (verbatim from exp168/169/170):
...
PREDECLARED PROPERTIES AND FALSIFIERS:
  ...
  P5 (tier-B claim, load-bearing): each pooled metric of bB — concentration,
    responsiveness count, valid margin, broken margin — within 0.05 of b8's
    SAME-RUN pooled values OR better.
  ...
  P3 reported-not-gating (declared, as exp170).
  P6 (straddle projection, third test, ungated): bB valid margin stays < -0.05.

  VERDICT:
    POSITIVE (TIER B SUPPORTED — N3 regulates its own evidence horizon from the
             stream's reliability and tempo at no cost vs the magic constant)
...
UNGATED DIAGNOSTICS:
  ...
  for both controllers; the P3/P6 report; everything exp170 logged.

--smoke: seed [130] only, full 24000 steps, prints schedule, K trajectory,
  both controllers' dial trajectories, per-segment scores for all three agents,
...
SEEDS = list(range(130, 138))   # fresh seeds — exp170 used 122-129
...
# TIER-B DERIVED K parameters for agent (bB)
# K(t) = clamp(ceil(2 * median(last M completed run lengths)) + 1, K_MIN, K_MAX)
# with no completed runs: K(t) = K_MIN
K_MULTIPLIER = 2                       # multiplier on the median
...
def compute_k_tier_b(completed_run_lengths: list[int]) -> int:
    """Compute the tier-B derived lock horizon K(t) for agent (bB).

    K(t) = clamp(ceil(K_MULTIPLIER * median(last K_WINDOW_M completed run lengths)) + 1,
                 K_MIN, K_MAX)
    with no completed runs: K(t) = K_MIN (= 3).

    Uses a sliding window of the last M=12 completions so K tracks the stream's
    CURRENT cycling tempo rather than the global history (tier-A's failure mode).
    The MEDIAN (not percentile) is outlier-robust: one long STRUCTURAL plateau
    completing at a regime boundary cannot ratchet the horizon.
    The multiplier 2 places the bar just beyond the typical demonstrated run.
    ...
    raw = math.ceil(K_MULTIPLIER * med) + 1
...
def run_fork(
    ...
    fork_name = f"exp171_s{fork_seed}"
...
def _run_n3_controller(
    ...
    *,
    ...
    def _get_k() -> int:
        if not use_tier_b_k:
            return LOCK_K_B8
        return compute_k_tier_b(completed_run_lengths)
...
def print_k_trajectory(run_results: list[dict], fork_scores: dict, seeds: list[int]) -> None:
    ...
    print("K(t) trajectories for bB (tier-B derived lock horizon — ungated diagnostic):")
    for seed in seeds:
        fs = fork_scores[seed]
        ...
        if completed:
            ...
            print(f"    median={med:.2f}  => ceil(2*med)+1={math.ceil(K_MULTIPLIER*med)+1}"
                  f"  => K=clamp({math.ceil(K_MULTIPLIER*med)+1},{K_MIN},{K_MAX})={k_final}")
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Exp 171 — K-endogenization tier B: recency-weighted, outlier-robust horizon. "
            "Three same-schedule arms: (a) W=200, (b8) K=8, (bB) tier-B median K."
        )
    )
    parser.add_argument(
        "--smoke", action="store_true",
        help=(
            "Smoke run: seed=[130] only, full 24000 steps, prints segment schedule, "
            "K trajectory (bB), both controllers' dial trajectories, per-segment scores "
            "for all three agents. No verdict."
        ),
    )
    args = parser.parse_args()

    smoke = args.smoke
    seeds = [130] if smoke else SEEDS

    print("=" * 80)
    print("Exp 171 — K-ENDOGENIZATION TIER B: recency-weighted outlier-robust horizon")
    print("          K(t) = clamp(ceil(2 * median(last 12 completed runs)) + 1, 3, 16)")
    print("          Three arms: (a) W=200, (b8) K=8, (bB) tier-B; same schedule/stream.")
    print("=" * 80)
    print()
    ...
    print(f"TIER-B K: multiplier={K_MULTIPLIER}  window_M={K_WINDOW_M}  K_MIN={K_MIN}  K_MAX={K_MAX}")
    print(f"  K(t) = clamp(ceil({K_MULTIPLIER} * median(last {K_WINDOW_M} runs)) + 1, {K_MIN}, {K_MAX})")
    ...
    if pc_failures:
        ...
        out_rows_path = Path(__file__).parent / "outputs" / "exp171_rows.json"
    ...
    print(f"P5 (TIER-B CLAIM: bB within {P5_TOLERANCE} of b8's SAME-RUN pooled values OR better):")
    ...
    print(f"P5  (tier-B: bB within {P5_TOLERANCE} of b8 same-run values or better): {p5_status}")
    ...
    if ev["verdict_str"] == "POSITIVE":
        ...
        print("TIER B SUPPORTED — N3 regulates its own evidence horizon from the stream's")
        print("reliability and tempo at no cost vs the magic constant (K=8 same-run arm).")
        print("The sliding-window median is outlier-robust and tempo-responsive.")
    ...
    arms = {
        ...
        "P6_straddle_projection_ungated": {
            ...
            "reason": (
                f"UNGATED. Projection: bB valid margin < {P6_VALID_MARGIN_THRESH}. "
    ...
    verdict_path = Path(__file__).parent / "outputs" / "exp171_verdict.json"
    write_verdict(
        path=verdict_path,
        experiment="exp171",
        arms=arms,
        ...
        halted=False,
        notes=(
            "N3 K-endogenization tier B: recency-weighted outlier-robust horizon. "
            f"K(t)=clamp(ceil({K_MULTIPLIER}*median(last {K_WINDOW_M} runs))+1,{K_MIN},{K_MAX}). "
            "Three arms on same schedule/stream: (a) W=200, (b8) K=8, (bB) tier-B. "
            ...
            "Seeds 130-137 (exp170 used 122-129). "
            f"POSITIVE: TIER B SUPPORTED. NEGATIVE: falsifier fired. "
