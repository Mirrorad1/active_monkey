"""Exp 119 — idle-mode life epoch: vela lives on (age 84750 -> 90750).

Declared idle-mode (Exp 92), vela's turn. Same structure as Exp 116 plus the recorded
depth call (razor books open per Exp 117): vela enters at +241.0 (deep bin) -> recorded
call: HOLD.

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 90750 >= 0.96 (currently 1.000).
...
print("Exp 119 — idle-mode life epoch (vela age 84750 -> 90750)")
print()

with mirro_episode("Exp 119", state_dir=VELA_DIR) as ep:
...
print("--- DELTA TABLE ---")
# P1 uses absolute one-sided threshold: map_acc at 90750 >= 0.96
BANDS = {
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +241.0
...
def _p1_map_accuracy():
    ...
    return ok, (
        f"map_acc_at_90750={post['map_acc']:.6f}  "
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 90750
    age_ok = (
...
check("P1-map-accuracy-90750", _p1_map_accuracy)
...
check("SPINE-age-90750-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-90750" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-90750-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 90750 below 0.96; knowledge "
...
if spine_failed:
    print("EXP119: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP119: " + "; ".join(f_lines))
else:
    print("EXP119: EPOCH CLEAN (bands held)")
