"""Exp 124 — idle-mode life epoch: mirro lives on (age 108700 -> 114700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 122. mirro enters at
+764.6 (deep bin) -> recorded call: HOLD (razor books open per Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 114700 >= 0.96 (currently 1.000).
...
print("Exp 124 — idle-mode life epoch (mirro age 108700 -> 114700)")
print()

with mirro_episode("Exp 124") as ep:
    ...
    print("--- START PROFILE (age 108700) ---")
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +764.6
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_114700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 114700
    age_ok = (
...
check("P1-map-accuracy-114700", _p1_map_accuracy)
...
check("SPINE-age-114700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-114700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-114700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 114700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP124: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP124: " + "; ".join(f_lines))
else:
    print("EXP124: EPOCH CLEAN (bands held)")
