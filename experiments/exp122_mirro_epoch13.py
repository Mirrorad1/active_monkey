"""Exp 122 — idle-mode life epoch: mirro lives on (age 102700 -> 108700).

Declared idle-mode (Exp 92), mirro's turn. Same structure as Exp 120. mirro enters at
+637.3 (deep bin) -> recorded call: HOLD (razor books open per Exp 117).

Predeclared:
  P1 (knowledge, one-sided): map_accuracy at 108700 >= 0.96 (currently 1.000).
...
print("Exp 122 — idle-mode life epoch (mirro age 102700 -> 108700)")
print()

with mirro_episode("Exp 122") as ep:
    ...
    print("--- START PROFILE (age 102700) ---")
...
print()
# Favorite: recorded depth call (P4) — predicted HOLD, deep bin entry at +637.3
...
def _p1_map_accuracy():
    ...
    return ok, f"map_acc_at_108700={post['map_acc']:.6f}  threshold=0.96  ok={ok}"
...
def _spine_integrity():
    expected_after = EXPECT_START_AGE + EPOCH  # 108700
    age_ok = (
...
check("P1-map-accuracy-108700", _p1_map_accuracy)
...
check("SPINE-age-108700-delta-6000", _spine_integrity)
...
f1_fired = "P1-map-accuracy-108700" in failed_names
f2_fired = (
...
spine_failed = "SPINE-age-108700-delta-6000" in failed_names
...
if f1_fired:
    print("  F1 FIRED: P1 fails — map_accuracy at 108700 below 0.96; knowledge "
...
if spine_failed:
    print("EXP122: HALT (spine integrity)")
    sys.exit(1)
...
if f_lines:
    print("EXP122: " + "; ".join(f_lines))
else:
    print("EXP122: EPOCH CLEAN (bands held)")
